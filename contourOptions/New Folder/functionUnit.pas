...
{      ALayerOptions := TLayerOptions.Create(myHandle,LayerHandle);
      ...
      AContour := TContourObjectOptions.Create(myHandle,LayerHandle,ContourIndex);
      try
        NodeCount := AContour.NumberOfNodes;
        ShowMessage(IntToStr(NodeCount));
        for Index := 0 to NodeCount-1 do
        begin
          AContour.GetNthNodeLocation(X,Y,Index);
...
begin
	...
	gPIEAbsFDesc.name := 'Delphi_PIE_GetObject';	        // name of function
	...
	gPIEAbsFDesc.paramTypes := nil;	// pointer to parameters types list
       	...
       	gPIEAbsDesc.name  := 'Delphi_PIE_GetObject';		// name of PIE
