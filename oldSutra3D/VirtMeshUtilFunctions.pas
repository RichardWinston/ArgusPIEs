...
procedure AssignNodeValues(ModelHandle: ANE_PTR; ThisContourType: integer;
  ListOfSegments : TList; SourceChoice, FollowMeshParamIndex : SmallInt;
  AValue, SubsidiaryValue : extended; LayerName : string;
  AContour : TContourObjectOptions; ALayer : TLayerOptions;
  SourceExpr, ConcExpr : string; SourceOverRide, ConcOverRide : boolean;
...
procedure AssignNodeValuesFromPlanes(ModelHandle: ANE_PTR; ListOfPlanes : TList;
  ...
  SourceExpr, ConcExpr : string; SourceOverRide, ConcOverRide : boolean;
  AddValues : boolean; const Comment : string);
...
begin
  ...
  {      if ListOfSegments.Count > 0 then
        begin
          ASegment := ListOfSegments[0];
          Min := ASegment.StartLoc[X];
          Max := ASegment.StartLoc[X];
          for SegmentIndex := 0 to ListOfSegments.Count -1 do
          begin
            ASegment := ListOfSegments[SegmentIndex];
            if ASegment.StartLoc[X] < Min then
            begin
              Min := ASegment.StartLoc[X]
            end;
            if ASegment.StartLoc[X] > Max then
            begin
              Max := ASegment.StartLoc[X]
            end;
          end;
            if ASegment.EndLoc[X] < Min then
            begin
              Min := ASegment.EndLoc[X]
            end;
            if ASegment.EndLoc[X] > Max then
            begin
              Max := ASegment.EndLoc[X]
            end;
        end;}
                ...
                begin
                  ...
                  begin
                    ...
                    BotElevValue := ALayer.RealValueAtXY
                      (ModelHandle, NodeX, NodeY, Expression);
                    NodeZ := ANode.Z;
                    if (TopElevValue >= NodeZ) and
                      (BotElevValue < NodeZ) then
                    begin
                      if not SourceOverRide then
                      begin
                        AValue := ALayer.RealValueAtXY(ModelHandle, ANode.X, ANode.Y,SourceExpr);
        ...
        try
          ...
          begin
            ...
            begin
              ...
              begin
                ANode.Value1 := ANode.Value1 +
                  AValue1*IntersectionPlane.Area/APlane.PlaneArea;
                ANode.Comment := ANode.Comment + ' ' + Comment;
              end
              else
              begin
            ...
            begin
              ...
              begin
                ANode.Value1 := ANode.Value1 +
                  AValue1*IntersectionPlane.Area;
                ANode.Comment := ANode.Comment + ' ' + Comment;
              end
              else
              begin
