...
{	/* Set the values of an object in a layer.
		...
		Input:
			layerHandle - the target layer, layerHandle=nil denotes current layer
...
function ANE_LayerGetObjectInfo(const handle : ANE_PTR;
         ...
         const AnObject ANE_PTR:
         numValues : ANE_INT16; valuesTypes : EPIENumberType_PTR;
         valuesIndices : ANE_INT16_PTR; values : ANE_PTR_PTR)
...
{ANE_BOOL ANE_LayerGetObjectInfo(ANE_CPTR handle, ANE_CPTR layerHandle,
  ...
  enum EPIENumberType* valuesTypes, ANE_INT16* valuesIndices, ANE_CPTR* values);
	/* Get the values of an object in a layer.
		...
		Input:
			layerHandle - the target layer, layerHandle=0 denotes current layer
			numValues - number of values to be set for the object
			valuesTypes - an array of length numValues, where each element represents the type of the value
			valuesIndices	- an array of length numValues, where each element holds the index of the parameter to get the value
			values - an array of pointers to data values. value[i] is a pointer to a value of type valuesTypes[i] to be filled with the value from parameter valuesIndices[i]

		Return Value:
			if the function succeeds, it fills in the proper data and returns true.
			it if fails, it returns false. for more information about the error, call ANE_GetLastError()
	*/
}
...
{
function ANE_ContourObjectGetNumVertices(const handle : ANE_PTR;
  const contourObject : ANE_PTR) : ANE_INT32;
          cdecl; external 'ANECB.dll' name 'ANE_ContourObjectGetNumVertices';
}

{ANE_INT32 ANE_ContourObjectGetNumVertices(ANE_CPTR handle, ANE_CPTR contourObject);}
...
procedure ANE_ContourObjectGetNthVertexLocation(const handle : ANE_PTR;
  const contourObject : ANE_PTR; n : ANE_INT32; x : ANE_DOUBLE_PTR;
  y: ANE_DOUBLE_PTR)
...
{void ANE_ContourObjectGetNthVertexLocation(ANE_CPTR handle,
  ANE_CPTR contourObject, ANE_INT32 n, ANE_DOUBLE* x, ANE_DOUBLE* y);}
