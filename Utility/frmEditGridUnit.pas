...
//    procedure ResetOutline;
  ...
  public
    LayerHandle : ANE_PTR;
    function GetGrid(LayerName : string) : boolean;
...
function TfrmEditGrid.GetGrid(LayerName : string) : boolean;
var
...
begin
  ...
  try
    ...
    try
      frmGridType.rgGridType.Caption := 'What type of grid is ' + LayerName + '?';
...
begin
  ...
  + 'cell and then select "Edit|Toggle Active".  Click the Help button to open '
  + 'the topic in the help file where these instructions are displayed.'
  , mtInformation, [mbOK, mbHelp], 137);
end;
...
begin
  ...
  try
    ...
    begin
      with frmRowColPositions.FrameColPosition do
      begin
        Multiplier := InternationalStrToFloat(adeMultiplier.Text);
        ColumnPositions.Clear;
        for Index := 0 to seCount.Value -1 do
        begin
          try
            ColumnPositions.Add(StrToFloat(dgPositions.Cells[1,Index])*Multiplier);
      ...
      begin
        ...
        begin
          try
            RowPositions.Add(StrToFloat(dgPositions.Cells[1,Index])*Multiplier);
          except on EConvertError do
            begin
            end;
          end;
...
begin
  ...
  Application.CreateForm(TfrmRowColPositions,frmRowColPositions);
  try
    frmRowColPositions.Caption := 'Edit row and column widths';
    with frmRowColPositions.FrameColPosition do
    begin
//      Multiplier := InternationalStrToFloat(adeMultiplier.Text);
      adeMultiplier.Text := '1';
    ...
    begin
      ...
      begin
        ...
        begin
          try
            AWidth := StrToFloat(dgPositions.Cells[1,Index]) * Multiplier;
            if ColumnsReversed then
            begin
              AWidth := StrToFloat(dgPositions.Cells[1,seCount.Value -1 - Index])
                * Multiplier;
              Sum := Sum - AWidth;
            end
            else
            begin
              AWidth := StrToFloat(dgPositions.Cells[1,Index])
                * Multiplier;
              Sum := Sum + AWidth;
            end;
...
begin
  ...
  try
    ...
    BlockCenteredGrid := frmGetLayerNames.cbBlockCenteredGrid.Checked;
  finally
  ...
  try
    ReadArgusContours(ALayer.Text[CurrentModelHandle],
      TEditGridContour,TEditGridPoint, Separator);
