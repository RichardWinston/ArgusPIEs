...
uses ANECB, JoinUnit, UtilityFunctions, OptionsUnit, ChooseLayerUnit,
  frmAboutUnit;
...
Procedure ExtractCoordFromString(AString : string; var XCoord, YCoord : double;
  const ExportDelimiter: Char = #9);
var
  TabPosition : integer;
begin
  TabPosition := Pos(ExportDelimiter,AString);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          AContour.Value := Copy(AContour.Value, Pos(Delimiter, AContour.Value)+ 1,
                            Length(AContour.Value));
          AContour.FixValue(aneHandle);
    ...
    try
      begin
        ...
        begin
            ...
            begin
              ExtractCoordFromString(AContour.Points.Strings
                [AContour.Points.Count -1], X, Y, Delimiter) ;
              FirstPoint.XCoord := X;
              FirstPoint.YCoord := Y;
              ExtractCoordFromString(AContour.Points.Strings
                [AContour.Points.Count -2], X, Y, Delimiter);
              ...
              begin
                ExtractCoordFromString(AContour.Points.Strings
                  [PointIndex], X, Y, Delimiter);
                ThirdPoint.XCoord := X;
                ThirdPoint.YCoord := Y;
                Segment1Length := Sqrt(Sqr(ThirdPoint.XCoord
                  - SecondPoint.XCoord)
                  + Sqr(ThirdPoint.YCoord - SecondPoint.YCoord));
                Segment2Length := Sqrt(Sqr(FirstPoint.XCoord
                  - SecondPoint.XCoord)
                  ...
                  begin
                    try
                      begin
                        ...
                        begin
                          Angle := Pi - Abs(ArcCos(Angle));
                        end
                      end;
                    except on Exception do
                      begin
                        ShowMessage(IntToStr(PointIndex) + ' '
                          + IntToStr(ContourIndex));
                      end;
                    end;
                  end;
                end;
      ...
      begin
        ...
        begin
          AContour.Heading.Strings[HeadingIndex + 1] :=
             IntToStr(AContour.Points.Count) + #9 + AContour.Value;
          break;
        end;
      end;
    end;
    ...
    begin
      AContour := ContourList.Items[index];
      AContour.Points.Add('');
      ContourStringList.Text := ContourStringList.Text
        + AContour.Heading.Text + AContour.Points.Text;
    end;
      ...
      begin
        ...
        try
          ...
          begin
            ...
            try
              if (Layer.LayerType[aneHandle] <> pieDomainLayer) then
              begin
...
begin
  ...
  begin
    HelpFile := HelpDirectory + '\Utility.chm'
  end;
end;

procedure TDeclutterForm.btnAboutClick(Sender: TObject);
begin
  inherited;
  ShowAbout;
end;
