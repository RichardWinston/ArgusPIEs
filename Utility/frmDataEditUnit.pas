...
uses
  ...
  ASLink, Spin, Math, WriteContourUnit, PointContourUnit;
...
type
  ...
  private
    ...
    function SetDataRows(const Importing: boolean): boolean;
  ...
  public
    ...
    procedure GetData(ADataLayerHandle: ANE_PTR; const Importing: boolean);
...
uses Clipbrd, ANECB, UtilityFunctions, CheckVersionFunction, ExportProgressUnit,
  IntListUnit;
...
begin
  ...
  StatusBar1.SimpleText := Format('X: %g; Y: %g', [CurrentPosition.X,
    CurrentPosition.Y]);
end;
...
//    tmrPaint.Enabled := True;
  ...
  else if btnLasso.Down and ShowSelectRect then
  begin
//    tmrPaint.Enabled := True;
//    zbDataPoints.ContinueZoom(X, Y);
    SelectRect.Right := X;
    SelectRect.Bottom := Y;
    shpChoose.Left := Min(SelectRect.Left, SelectRect.Right);
    shpChoose.Top := Min(SelectRect.Top, SelectRect.Bottom);
    shpChoose.Width := Abs(SelectRect.Left - SelectRect.Right);
    shpChoose.Height := Abs(SelectRect.Top - SelectRect.Bottom);
    shpChoose.Visible := True;
//    zbDataPoints.Invalidate;
  end;
end;
...
//  ZP_YCoord: integer;
  ...
  XCoord: Integer;
  YCoord: Integer;
  ExpProgFrom: TfrmExportProgress;
  IntList: TIntegerList;
  PointsToDelete: TList;
  ZoomIndex: Integer;
  PointsArray: array of TRbwZoomPoint;
  RowIndex: Integer;
  RowToDelete: Integer;
  CurrentRow: Integer;
  ColIndex: Integer;
begin
  ...
  begin
    shpChoose.Visible := False;
//    zbDataPoints.CancelZoom;
    SelectRect.Right := X;
    SelectRect.Bottom := Y;
    if SelectRect.Left > SelectRect.Right then
    begin
      Temp := SelectRect.Left;
      SelectRect.Left := SelectRect.Right;
      SelectRect.Right := Temp;
    end;
    if SelectRect.Top > SelectRect.Bottom then
    begin
      Temp := SelectRect.Top;
      SelectRect.Top := SelectRect.Bottom;
      SelectRect.Bottom := Temp;
    end;
    for Index := 0 to ZoomPoints.Count - 1 do
    begin
      ZoomPoint := ZoomPoints[Index] as TSelectZoomPoint;
      XCoord := ZoomPoint.XCoord;
      YCoord := ZoomPoint.YCoord;
      ZoomPoint.Selected := (SelectRect.Top <= YCoord)
        and (SelectRect.Bottom >= YCoord)
        and (SelectRect.Left <= XCoord)
        and (SelectRect.Right >= XCoord)
    end;
    zbDataPoints.Invalidate;
    if (MessageDlg('Do you want to delete the selected points?',
      mtInformation, [mbYes, mbNo], 0) = mrYes) then
    begin
      IntList := TIntegerList.Create;
      PointsToDelete := TList.Create;
      ExpProgFrom := TfrmExportProgress.Create(nil);
      try
        ExpProgFrom.ProgressBar1.Max := ZoomPoints.Count;
        ExpProgFrom.ProgressBar1.Position := 0;
        ExpProgFrom.Show;

        for Index := 0 to ZoomPoints.Count - 1 do
        begin
          ExpProgFrom.ProgressBar1.StepIt;
          ZoomPoint := ZoomPoints[Index] as TSelectZoomPoint;
          if ZoomPoint.Selected then
          begin
            IntList.Add(Index);
            PointsToDelete.Add(ZoomPoint);
          end;
        end;
        SetLength(PointsArray, PointsToDelete.Count);
        ExpProgFrom.ProgressBar1.Position := 0;
        ExpProgFrom.ProgressBar1.Max := PointsToDelete.Count;
        for ZoomIndex := 0 to PointsToDelete.Count - 1 do
        begin
          ZoomPoint := PointsToDelete[ZoomIndex];
          PointsArray[ZoomIndex] := ZoomPoint;
          ExpProgFrom.ProgressBar1.StepIt;
        end;
        zbDataPoints.DeletePoints(PointsArray);

        ExpProgFrom.ProgressBar1.Position := 0;
        for ZoomIndex := 0 to IntList.Count - 1 do
        begin
          ExpProgFrom.ProgressBar1.StepIt;
          Index := IntList[ZoomIndex];

          ZoomPoints[Index] := nil;
          DataValues[Index] := nil;
//          if dgDataPoints.RowCount = 2 then
//          begin
//            dgDataPoints.FixedRows := 0;
//          end;
//          dgDataPoints.DeleteRow(Index + 2);
        end;
        ZoomPoints.Pack;
        DataValues.Pack;
        ...
        begin
          ExpProgFrom.ProgressBar1.Position := 0;
          Index := 0;
          RowToDelete := IntList[0]+2;
          ExpProgFrom.ProgressBar1.Max := dgDataPoints.RowCount-2;
          CurrentRow := 2;
          for RowIndex := 2 to dgDataPoints.RowCount - 1 do
          begin
            ExpProgFrom.ProgressBar1.StepIt;
            if RowToDelete = RowIndex then
            begin
              Inc(Index);
              if Index < IntList.Count then
              begin
                RowToDelete := IntList[0]+2;
              end
              else
            ...
            begin
              ...
              begin
                for ColIndex := 0 to dgDataPoints.ColCount - 1 do
                begin
                  dgDataPoints.Cells[ColIndex,CurrentRow]
          ...
          dgDataPoints.RowCount := CurrentRow;
          if dgDataPoints.RowCount = 2 then
          begin
            dgDataPoints.FixedRows := 0;
          end;
...
//        for Index := ZoomPoints.Count - 1 downto 0 do
//        begin
//          ExpProgFrom.ProgressBar1.StepIt;
//          ZoomPoint := ZoomPoints[Index] as TSelectZoomPoint;
//          if ZoomPoint.Selected then
//          begin
//            ZoomPoints.Delete(Index);
//            DataValues.Delete(Index);
//            if dgDataPoints.RowCount = 2 then
//            begin
//              dgDataPoints.FixedRows := 0;
//            end;
...
procedure TfrmDataEdit.GetData(ADataLayerHandle: ANE_PTR; const Importing:
  boolean);
const
  MajorVersion = 4;
  MinorVersion = 2;
...
begin
  ...
  try
    ...
    if OK_Version then
    begin
      zbDataPoints.VerticalExaggeration :=
        DataLayer.CoordXYRatio[CurrentModelHandle];
    end;

    Ok := SetDataColumns(False);
    if not OK then
      Exit;
    Ok := SetDataRows(Importing);
    if not OK then
      Exit;
    Count := 0;
...
begin
  ...
  CurrentPosition.UseForZoomOut := False;
  ...
  Label1.Caption := 'Version: ' + FileVersion(GetDLLName);
...
function TfrmDataEdit.SetDataRows(const Importing: boolean): boolean;
var
...
begin
  ...
  try
    ...
    if not Importing and (dgDataPoints.RowCount < 3) then
    begin
      Beep;
...
begin
  ...
  try
    ...
    if DataValues.Capacity < ObjectCount then
    begin
      DataValues.Capacity := ObjectCount;
    end;
...
begin
  inherited;
//  if btnLasso.Down and ShowSelectRect then
//  begin
//    zbDataPoints.PBCanvas.Brush.Style := bsClear;
//    zbDataPoints.PBCanvas.Rectangle(SelectRect);
//  end;
  for Index := 0 to ZoomPoints.Count - 1 do
  begin
    ZoomPoint := ZoomPoints[Index] as TSelectZoomPoint;
    ...
    begin
      ...
      zbDataPoints.PBCanvas.Rectangle(XCoord - 2, YCoord - 2, XCoord + 2, YCoord
        + 2);
    end;
  end;
end;
        ...
        begin
          ...
          begin
            ...
            if (DataValue.X > MaxX) or (DataValue.X < MinX)
              or (DataValue.Y > MaxY) or (DataValue.Y < MinY) then
              ShowWarning := True;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            AStringGrid.OnSetEditText(AStringGrid, 0, RowIndex,
              AStringGrid.Cells[0, RowIndex]);
          end;
          AStringGrid.Cells[1, RowIndex] := FloatToStr(APoint.X);
          if Assigned(AStringGrid.OnSetEditText) then
          begin
            AStringGrid.OnSetEditText(AStringGrid, 1, RowIndex,
              AStringGrid.Cells[1, RowIndex]);
          end;
          AStringGrid.Cells[2, RowIndex] := FloatToStr(APoint.Y);
          if Assigned(AStringGrid.OnSetEditText) then
          begin
            AStringGrid.OnSetEditText(AStringGrid, 2, RowIndex,
              AStringGrid.Cells[2, RowIndex]);
          end;
          ...
          begin
            ...
            if Assigned(AStringGrid.OnSetEditText) then
            begin
              AStringGrid.OnSetEditText(AStringGrid, ValueIndex + 3, RowIndex,
                AStringGrid.Cells[ValueIndex + 3, RowIndex]);
            end;
          end;
        end;
...
procedure TfrmDataEdit.btnReadFileClick(Sender: TObject);
begin
  inherited;
  ReadData(dsFile);
  StoreContourTitles;
