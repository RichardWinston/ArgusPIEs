  ...
  try
    ProjectOptions.LayerNames(CurrentModelHandle,
      [pieTriMeshLayer,pieQuadMeshLayer,pieInformationLayer,
        pieGridLayer,pieDataLayer,pieDomainLayer],clbLayerNames.Items);
  finally
    ProjectOptions.Free;
  end;
    ...
    begin
      ...
      begin
        Layer := T_ANE_NamedInfoLayer.Create(
          clbLayerNames.Items[Index], nil, -1);
        LayerOption := TLayerOptions.CreateWithName(clbLayerNames.Items[Index],
          CurrentModelHandle);
        try
...
procedure TfrmAddParameters.btnAboutClick(Sender: TObject);
begin
  inherited;
  ShowAbout;
