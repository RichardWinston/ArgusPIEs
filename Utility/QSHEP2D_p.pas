...
type
  ...
  T5SingleArray = array[0..4] of TSingleArray;

procedure TestIt;

procedure QSHEP2(var N: integer; var X, Y, F: TSingleArray; var NQ, NW,
  NR : integer; var LCELL: T2DIntegerArray; var LNEXT: TIntegerArray;
  var XMIN, YMIN, DX, DY, RMAX: single; var RSQ: TSingleArray;
  var A: T5SingleArray; var IER: integer);

FUNCTION QS2VAL (var PX,PY: single; var N: integer; var X,Y,F: TSingleArray;
  var NR: integer; var LCELL: T2DIntegerArray; var LNEXT: TIntegerArray;
  var XMIN, YMIN,DX,DY,RMAX: single; var RSQ: TSingleArray;
  var A: T5SingleArray; out PointsFound: boolean): single;

procedure STORE2 (var N: integer; var X,Y: TSingleArray;var NR: integer;
  var LCELL: T2DIntegerArray; var LNEXT: TIntegerArray; var XMIN,YMIN,DX,
      DY: single; var IER: integer);

procedure GETNP2 (const PX,PY: single; const X,Y: TSingleArray; const NR: integer;
  const LCELL: T2DIntegerArray; const LNEXT: TIntegerArray;
  const XMIN,YMIN,DX,DY: single; var NP: integer; var DSQ: single);

procedure SETUP2 (var XK,YK,FK,XI,YI,FI,S1,S2,R: single; var ROW: array of single);

procedure GIVENS (var A,B, C, S: single);

procedure ROTATE (const N: integer; var C,S: single; var X,Y: array of single);

FUNCTION STORE (var X: single): single;
...
procedure QSHEP2(var N: integer; var X, Y, F: TSingleArray; var NQ, NW,
  ...
  var A: T5SingleArray; var IER: integer);
var
  I, IB, IERR, IP1, IRM1, IROW, J, JP1, K, LMAX,
  LNP, NEQ, NN, NNQ, NNR, NNW, NP,
  NQWMAX: INTEGER;
  NPTS: array[0..39] of integer;
  AV, AVSQ, C, DDX, DDY, DMIN, FK,
  RQ, RS, RSMX, RSOLD, RWS, S, SUM, T,
  XK, XMN, YK, YMN: single;
  B: array[0..5,0..5] of single;
  ARow: array[0..5] of single;
  Row1, Row2: array[0..5] of single;
  RowIndex: integer;
  TempI: integer;
  label label1, label2, label3, label4, label5, label7, label8, label9,
    label10, label13, label15, label20, label21, label22;
const
  RTOL: single = 1e-5;
  DTOL: single = 0.001;
  SF: single = 1;
begin
      ...
      NQWMAX := MAX(NNQ,NNW);
      LMAX := MIN(40,NN-1);
      IF (5 > NNQ)  or  (1 > NNW)	or  (NQWMAX > LMAX)
      	or  (NNR < 1) then GOTO label20;
...
// OUTER LOOP ON NODE K
      ...
      // COMPUTE NPTS, LNP, RWS, NEQ, RQ, AND AVSQ.
          ...
          label1:	SUM := SUM + RS;
          IF (LNP = LMAX) then GOTO label3;
          LNP := LNP + 1;
          RSOLD := RS;
          GETNP2 (XK,YK,X,Y,NNR,LCELL,LNEXT,XMN,YMN,
           		       DDX,DDY, NP,RS);
          IF (RS = 0.0) then GOTO label21;
          NPTS[LNP-1] := NP;
          IF ( (RS-RSOLD)/RS < RTOL ) then GOTO label1;
          IF (RWS = 0.0)  AND  (LNP > NNW) then RWS := RS;
          IF (RQ <> 0.0)  OR	(LNP <= NNQ) then GOTO label2;
      ...
      //   BOTTOM OF LOOP -- TEST FOR TERMINATION.
          ...
          label2:	  IF (LNP > NQWMAX) then GOTO label4;
          GOTO label1;
      ...
      //   DISTANCE RS TO THE LAST NODE INCLUDED.
          ...
          label3:	IF (RWS = 0.0) then RWS := 1.1*RS;
        IF (RQ <> 0.0) then GOTO label4;
          NEQ := LMAX;
          RQ := SQRT(1.1*RS);
          AVSQ := SUM/NEQ;
      ...
      //   WITH ORTHOGONAL MATRIX Q NOT STORED.
        ...
        I := 0;
          ...
          begin
            ARow[RowIndex] := B[RowIndex,IROW-1];
          end;

          SETUP2 (XK,YK,FK,X[NP-1],Y[NP-1],F[NP-1],AV,AVSQ,
           		       RQ, ARow);
          for RowIndex := 0 to 5 do
          begin
            B[RowIndex,IROW-1] := ARow[RowIndex];
          end;
          IF (I = 1) then GOTO label5;
          IRM1 := IROW-1;
          for J := 1 to IRM1 do
          begin
            JP1 := J + 1;
            GIVENS (B[J-1,J-1],B[J-1,IROW-1],C,S);
            for RowIndex := 0 to 6-J-1 do
            begin
              Row1[RowIndex] := B[JP1-1+RowIndex,J-1];
              Row2[RowIndex] := B[JP1-1+RowIndex,IROW-1];
            end;
            ROTATE (6-J,C,S,Row1,Row2);
            for RowIndex := 0 to 6-J-1 do
            begin
              B[JP1-1+RowIndex,J-1]      := Row1[RowIndex];
              B[JP1-1+RowIndex,IROW-1] := Row2[RowIndex];
            end;
          end;
          IF (I < NEQ) then GOTO label5;
      
      // TEST THE SYSTEM FOR ILL-CONDITIONING.

        DMIN := MIN( ABS(B[0,0]),ABS(B[1,1]));
        DMIN := MIN(DMIN,ABS(B[2,2]));
        DMIN := MIN(DMIN,ABS(B[3,3]));
        DMIN := MIN(DMIN,ABS(B[4,4]));
        IF (DMIN*RQ >= DTOL) then GOTO label13;
        IF (NEQ = LMAX) then GOTO label10;
      ...
      //   IS ALSO INCREASED IF NECESSARY.
          ...
          label7:	RSOLD := RS;
        NEQ := NEQ + 1;
        IF (NEQ = LMAX) then GOTO label9;
        IF (NEQ = LNP) then GOTO label8;
      
      //   NEQ .LT. LNP

        NP := NPTS[NEQ+1-1];
        RS := Sqr(X[NP-1]-XK) + Sqr(Y[NP-1]-YK);
        IF ( (RS-RSOLD)/RS < RTOL ) then GOTO label7;
        RQ := SQRT(RS);
        GOTO label5;
      ...
      //   ADD AN ELEMENT TO NPTS.
        ...
        GETNP2 (XK,YK,X,Y,NNR,LCELL,LNEXT,XMN,YMN,
           		     DDX,DDY, NP,RS);
        IF (NP = 0) then GOTO label21;
        NPTS[LNP-1] := NP;
        IF ( (RS-RSOLD)/RS < RTOL ) then GOTO label7;
        RQ := SQRT(RS);
        GOTO label5;

          label9:	RQ := SQRT(1.1*RS);
        GOTO label5;
      ...
      //   THREE EQUATIONS.
         ...
         begin
           ...
           begin
             JP1 := J + 1;
             GIVENS (B[J-1,J-1],B[J-1,5],C,S);
             for RowIndex := 0 to 6-J-1 do
             begin
               Row1[RowIndex] := B[JP1-1+RowIndex,J-1];
               Row2[RowIndex] := B[JP1-1+RowIndex,5];
             end;
             ROTATE (6-J,C,S,Row1,Row2);
             for RowIndex := 0 to 6-J-1 do
             begin
               B[JP1-1+RowIndex,J-1] := Row1[RowIndex];
               B[JP1-1+RowIndex,5]   := Row2[RowIndex];
             end;
           end;
         end;
      ...
      // SOLVE THE 5 BY 5 TRIANGULAR SYSTEM FOR THE COEFFICIENTS
         ...
         label13: for IB := 1 to 5 do
         begin
          I := 6-IB;
          T := 0.0;
          IF (I = 5) then GOTO label15;
          IP1 := I + 1;
          for J := IP1 to 5 do
          begin
            T := T + B[J-1,I-1]*A[J-1,K-1];
          end;
          label15:	  A[I-1,K-1] := (B[5,I-1]-T)/B[I-1,I-1];
         end;
...
begin
      NN := N;
      NNR := NR;
      IF (NN < 2)  or  (NNR < 1) then GOTO label4;
...
// COMPUTE CELL DIMENSIONS AND TEST FOR ZERO AREA.
      ...
      DX := DELX;
      DY := DELY;
      IF (DELX = 0)	or  (DELY = 0) then GOTO label5;
...
// LOOP ON NODES, STORING INDICES IN LCELL AND LNEXT.
      ...
      begin
        KB := NP1 - K;
        I := Trunc((X[KB-1]-XMN)/DELX) + 1;
        IF (I > NNR) then I := NNR;
        J := Trunc((Y[KB-1]-YMN)/DELY) + 1;
        IF (J > NNR) then J := NNR;
        L := LCELL[I-1,J-1];
        LNEXT[KB-1] := L;
        IF (L = 0) then LNEXT[KB-1] := KB;
        LCELL[I-1,J-1] := KB;
    end;
...
var
  FIRST: boolean;
  XP, YP: single;
  IMIN, IMAX, JMIN, JMAX, I0, J0, I1, I2, J1, J2: integer;
  DELX, DELY: single;
  J, I: integer;
  L: integer;
  LN: integer;
  ...
  LMIN: integer;
  R: single;
  label label1, label2, label3, label4, label7, label8, label9;
begin
      XP := PX;
      YP := PY;
...
// TEST FOR INVALID INPUT PARAMETERS.
      ...
      IF (NR < 1)  OR  (DX <= 0.0)  OR  (DY <= 0.0) then GOTO label9;
...
//   THOSE OUTSIDE THE RANGE (IMIN,IMAX) X (JMIN,JMAX).
    ...
    label1: for J := J1 to J2 do
    begin
      IF (J > JMAX) then GOTO label7;
      IF (J < JMIN) then Continue;
      for I := I1 to I2 do
      begin
        IF (I > IMAX) then Break;
        IF (I < IMIN) then Continue;
        IF (J <> J1)  AND	(J <> J2)  AND  (I <> I1)
         	      AND  (I <> I2) then Continue;
    ...
    //   LOOP ON NODES IN CELL (I,J).
        ...
        label2:	  LN := LNEXT[L-1];
        IF (LN < 0) then GOTO label4;

    //   NODE L IS NOT MARKED.

        RSQ := Sqr(X[L-1]-XP) + Sqr(Y[L-1]-YP);
        IF (NOT FIRST) then GOTO label3;
     ...
     //    IS RESET TO FALSE.
        ...
        RSMIN := RSQ;
        R := SQRT(RSMIN);
        IMIN := Trunc((DELX-R)/DX) + 1;
        IF (IMIN < 1) then IMIN := 1;
        IMAX := Trunc((DELX+R)/DX) + 1;
        IF (IMAX > NR) then IMAX := NR;
        JMIN := Trunc((DELY-R)/DY) + 1;
        IF (JMIN < 1) then JMIN := 1;
        JMAX := Trunc((DELY+R)/DY) + 1;
    ...
    //   TEST FOR NODE L CLOSER THAN LMIN TO P.
        ...
        label3:	  IF (RSQ > RSMIN) then GOTO label4;
    ...
    //   TEST FOR TERMINATION OF LOOP ON NODES IN CELL (I,J).
        ...
        label4:	  IF (ABS(LN) = L) then Continue;
        L := ABS(LN);
        GOTO label2;
        end;
    end;
...
// TEST FOR TERMINATION OF LOOP ON CELL LAYERS.
    ...
    label7: IF (I1 <= IMIN)	AND  (I2 >= IMAX)  AND
     	  (J1 <= JMIN)	AND  (J2 >= JMAX) then GOTO label8;
...
//   CLOSEST UNMARKED NODE TO P.
    ...
    label8: IF (FIRST) then GOTO label9;
      NP := LMIN;
      DSQ := RSMIN;
      LNEXT[LMIN-1] := -LNEXT[LMIN-1];
      Exit;
...
var
  I: Integer;
  DX, DY, DXSQ, DYSQ, D, W, W1, W2: single;
  label label1;
begin
      DX := XI - XK;
      DY := YI - YK;
      ...
      DYSQ := DY*DY;
      D := SQRT(DXSQ + DYSQ);
      IF (D <= 0.0)  OR  (D >= R) then GOTO label1;
      W := (R-D)/R/D;
      W1 := W/S1;
      W2 := W/S2;
      ROW[0] := DXSQ*W2;
...
begin
      AA := A;
      BB := B;
      IF (ABS(AA) <= ABS(BB)) then GOTO label1;
...
// ABS(A) .GT. ABS(B)
      ...
      V := BB/U;
      Except
        ShowMessage(FloatToStr(log10(U)));
        raise
      end;
      R := SQRT(0.25 + V*V) * U;
      C := AA/R;
...
// ABS(A) .LE. ABS(B)
    ...
    label1: IF (BB = 0.0) then GOTO label2;
      U := BB + BB;
      V := AA/U;
...
var
  ...
  XI, YI: single;
begin
  IF (N < 0) OR ((C = 1.0) AND (S = 0.0)) then Exit;
  for I := 1 to N do
  begin
    XI := X[I-1];
    YI := Y[I-1];
...
var
  ...
  SW, SWQ: single;
  J, I, K, KP: integer;
  DELX, DELY, DXSQ, DYSQ, DS, RS, RDS, RD, W: single;
  label label1, label2, label4, label5;
begin
  PointsFound := True;
      XP := PX;
      YP := PY;
      IF (N < 6)  OR  (NR < 1)  or  (DX <= 0.0)  OR
     	  (DY <= 0.0)  OR  (RMAX < 0.0) then Exit;
...
//   CENTERED AT P.
      ...
      IMIN := Trunc((XP-XMIN-RMAX)/DX) + 1;
      IMAX := Trunc((XP-XMIN+RMAX)/DX) + 1;
      IF (IMIN < 1) then IMIN := 1;
      IF (IMAX > NR) then IMAX := NR;
      JMIN := Trunc((YP-YMIN-RMAX)/DY) + 1;
      JMAX := Trunc((YP-YMIN+RMAX)/DY) + 1;
      IF (JMIN < 1) then JMIN := 1;
      IF (JMAX > NR) then JMAX := NR;
...
//   OF RADIUS RMAX.
      ...
      IF (IMIN > IMAX)  OR	(JMIN > JMAX) then GOTO label5;
...
// OUTER LOOP ON CELLS (I,J).
      ...
      // INNER LOOP ON NODES K.
          ...
          label1:	  DELX := XP - X[K-1];
          DELY := YP - Y[K-1];
          DXSQ := DELX*DELX;
          DYSQ := DELY*DELY;
          DS := DXSQ + DYSQ;
          RS := RSQ[K-1];
          IF (DS >= RS) then GOTO label2;
          IF (DS = 0.0) then GOTO label4;
          RDS := RS*DS;
          RD := SQRT(RDS);
          W := (RS+DS-RD-RD)/RDS;
          SW := SW + W;
          SWQ := SWQ + W*(A[0,K-1]*DXSQ + A[1,K-1]*DELX*DELY +
           			 A[2,K-1]*DYSQ + A[3,K-1]*DELX +
           			 A[4,K-1]*DELY + F[K-1]);
...
procedure QS2GRD (var PX,PY: single; var N: integer; var X,Y,F: TSingleArray;
  var NR: integer; var LCELL: T2DIntegerArray; var LNEXT: TIntegerArray;
  var XMIN, YMIN,DX,DY,RMAX: single; var RSQ: TSingleArray;
  var A: T5SingleArray; var Q,QX,QY: single; var IER: integer);
var
  XP, YP: single;
  IMIN, IMAX, JMIN, JMAX: integer;
  SW, SWX, SWY, SWQ, SWQX, SWQY: single;
  J, I, K: integer;
  DELX, DELY, DXSQ, DYSQ, DS, RS, RDS, RD, W, T, WX, WY, QKX, QKY, QK: single;
  KP: integer;
  SWS: single;
...
begin
      ...
      YP := PY;
      IF (N < 6)  OR  (NR < 1)  OR  (DX <= 0.0)  OR
     	  (DY <= 0.0)  OR  (RMAX < 0.0) then GOTO label5;
...
//   OF RADIUS RMAX.
      ...
      IF (IMIN > IMAX)  OR	(JMIN > JMAX) then GOTO label6;
...
// OUTER LOOP ON CELLS (I,J).
    ...
    // INNER LOOP ON NODES K.
        ...
        WX := DELX*T;
        WY := DELY*T;
        QKX := 2.*A[0,K-1]*DELX + A[1,K-1]*DELY;
        QKY := A[1,K-1]*DELX + 2.*A[2,K-1]*DELY;
        QK := (QKX*DELX + QKY*DELY)/2.0;
        QKX := QKX + A[3,K-1];
        QKY := QKY + A[4,K-1];
        QK := QK + A[3,K-1]*DELX + A[4,K-1]*DELY + F[K-1];
        SW := SW + W;
        SWX := SWX + WX;
        SWY := SWY + WY;
        SWQ := SWQ + W*QK;
...
var
  ...
  IER: integer;
  EPS, EP1: single;
  EQ, EQX, EQY, PY, PX, Q1, Q, QX, QY: single;
  RQ: single;
  PointsFound: boolean;
  label label4, label5, label6, label7, label8;
// QUADRATIC TEST FUNCTION AND PARTIAL DERIVATIVES
  function    FQ(var XX,YY: single): single;
  begin
    result := Sqr((XX + 2.*YY)/3.0);
  end;
  function    FX(var XX,YY: single): single;
  begin
    result := 2.*(XX + 2.0*YY)/9.0;
  end;
  function    FY(var XX,YY: single): single;
  begin
    result := 4.*(XX + 2.0*YY)/9.0;
  end;
...
// COMPUTE THE DATA VALUES.
      for K := 1 to N do
      begin
        F[K-1] := FQ(X[K-1],Y[K-1]);
      end;

// COMPUTE PARAMETERS DEFINING THE INTERPOLANT Q.

      QSHEP2 (N,X,Y,F,NQ,NW,NR, LCELL,LNEXT,XMIN,YMIN,
          DX,DY,RMAX,RSQ,A,IER);
      IF (IER <> 0) then GOTO label6;
...
// COMPUTE THE MACHINE PRECISION EPS.
    ...
    label4: EPS := EPS/2.0;
      EP1 := EPS + 1.0;
      IF (STORE(EP1) > 1.0) then GOTO label4;
      EPS := EPS*2.0;
      ...
      begin
        ...
        begin
          ...
          Q1 := QS2VAL (PX,PY,N,X,Y,F,NR,LCELL,LNEXT,XMIN,
           		       YMIN,DX,DY,RMAX,RSQ,A, PointsFound);
                QS2GRD (PX,PY,N,X,Y,F,NR,LCELL,LNEXT,XMIN,
           		       YMIN,DX,DY,RMAX,RSQ,A, Q,QX,QY,IER);
          IF (IER <> 0) then GOTO label7;
          IF (ABS(Q1-Q) > 3.*ABS(Q)*EPS) then GOTO label8;
...
// PRINT ERRORS AND THE RATIO EQ/EPS.
      ...
      RQ := EQ/EPS;
      ShowMessage('MAXIMUM ABSOLUTE ERRORS IN THE INTERPOLANT Q AND PARTIAL '
        + ' DERIVATIVES QX AND QY RELATIVE TO MACHINE PRECISION EPS'
        + #13#10
        + 'FUNCTION' + #9 + 'MAX ERROR' + #9 + 'MAX ERROR/EPS'

        + #13#10#13#10
        + 'Q' + #9 + FloatToStr(EQ) + #9 + FloatToStr(RQ)
        + #13#10
        + 'QX' + #9 + FloatToStr(EQX)
        + #13#10
        + 'QY' + #9 + FloatToStr(EQY)
...
C ERROR IN QSHEP2
C    }
   label6: ShowMessage('ERROR IN QSHEP2 -- IER = ' + IntToStr(IER));
   Exit;
{C
C ERROR IN QS2GRD
C     }
    label7: ShowMessage('ERROR IN QS2GRD -- IER = ' + IntToStr(IER));
    Exit;
{C
C VALUES RETURNED BY QS2VAL AND QS2GRD DIFFER BY A RELATIVE
C   AMOUNT GREATER THAN 3*EPS.
C   }
    label8: ShowMessage('ERROR -- INTERPOLATED VALUES: Q1 (QS2VAL) AND Q2 (QS2GRD) DIFFER --  '
      + 'Q1 = ' + FloatToStr(Q1) + ' Q2 = '  + FloatToStr(Q));
end;
