...
begin
  ...
  begin
    lrxopen2(ierror,UnitNumber, OpenDialog1.FileName, filenameLength);
  end
  else
  begin
    lrfopen2(ierror,UnitNumber, OpenDialog1.FileName, filenameLength);
  end;
end;
...
begin
  ...
  begin
    ...
    try
        ...
        try
          try
            repeat
              ...
              begin
                ReadMeFormatted2(InUnit,ierror, KSTP,KPER,
                  PERTIM,TOTIM,NCOL,NROW,ILAY,Text, Length(Text));
              end;
              if ierror = 0 then
              begin
      ...
      begin
        Beep;
        raise EInvalidData.Create('Error opening data file.');
      end;
    finally
      CloseFile(AFile);
    end;
