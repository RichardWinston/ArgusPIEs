...
uses
  ...
  RzTreeVw,
  {$ENDIF}
  {$IFDEF VirtualStringTreeView}
  VirtualTrees,
  {$ENDIF}
  {$IFDEF FusedControls}
  FusedTrctrls,
  {$ENDIF}
  {$IFDEF RbwCheckTreeView}
  RbwCheckTreeView,
  {$ENDIF}
  JvToolEdit,
  ComCtrls, checklst, RbwDataGrid2, RbwDataGrid4;

  {TArgusForm is an ancestor of forms used in a variety of PIEs. Its most
...
type
  ...
  // component has been removed or replaced and its data is stored in
  // some other control.
...
{$ENDIF}
  {$IFDEF RzTree}
    procedure ReadRzCheckTree(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
  ...
  {$ENDIF}
    ...
    procedure writeStringGrid3D(ComponentData: TStringList;
      AComponent: TComponent);
   {$IFDEF RzTree}
    procedure WriteRzCheckTree(ComponentData: TStringList;
      AComponent: TComponent);
   {$ENDIF}
    procedure ReadCheckListBox(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteCheckListBox(ComponentData: TStringList;
      AComponent: TComponent);
    procedure ReadRbwDataGrid2(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteRbwDataGrid2(ComponentData: TStringList;
      AComponent: TComponent);
    procedure ReadRbwDataGrid4(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteRbwDataGrid4(ComponentData: TStringList;
      AComponent: TComponent);
    ...
    {$IFDEF VirtualStringTreeView}
    procedure ReadVirtualStringTree(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteVirtualStringTree(ComponentData: TStringList;
      AComponent: TComponent);
    {$ENDIF}
    {$IFDEF RbwCheckTreeView}
    procedure ReadRbwCheckTreeView(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteRbwCheckTreeView(ComponentData: TStringList;
      AComponent: TComponent);
    {$ENDIF}
  private
    ...
    procedure ReadAComponent(var LineIndex: integer; FirstList,
      ...
      DataToRead: TStringList; const VersionControl: TControl);
{$IFDEF FusedControls}
    procedure ReadFusedRadioGroup(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteFusedRadioGroup(ComponentData: TStringList;
      AComponent: TComponent);
{$ENDIF}
    procedure ReadJvFilenameEdit(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent;
      DataToRead: TStringList; const VersionControl: TControl);
    procedure WriteJvFilenameEdit(ComponentData: TStringList;
      AComponent: TComponent);
    procedure ReadTreeView(var LineIndex: integer; FirstList,
      IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
      const VersionControl: TControl);
    procedure WriteTreeView(ComponentData: TStringList; AComponent: TComponent);
...
//    constructor Create(AOwner: TComponent); override;
    ...
    constructor Create(AOwner: TComponent); override;
    {$ENDIF}
    procedure SetControlColor(const AControl: TControl);
...
FileCtrl, UtilityFunctions, ExportProgressUnit, ANECB, JvVersionInfo;

{$R *.DFM}


class function TArgusForm.GetFormFromArgus(aneHandle : ANE_PTR {ANE_PTR_PTR}) : TArgusForm;
var
  APtr : ANE_PTR;
begin
  ANE_GetPIEProjectHandle(aneHandle, APtr );
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (AControl = VersionControl) or
                (OldVersionControlName.IndexOf(AName)>-1) then
              begin
            ...
            ReadComponents(LineIndex, FirstList, IgnoreList,
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            // to ReplaceTemplateFiles and then add its lines to
...
//  StringListIndex : integer;
  ...
  AText : String;
{$IFDEF FusedControls}
  AFusedRadioGroup : TFusedRadioGroup;
{$ENDIF}
  AFileEdit: TJvFilenameEdit;
  function Prefix(const AComponent : TComponent): string;
  var
    AnOwner : TComponent;
  begin
    result := '';
    AnOwner := AComponent.Owner;
    while AnOwner is TFrame do
    begin
      result := AnOwner.Name + '.';
      AnOwner := AnOwner.Owner;
    end;
  end;
  Procedure ProcessComponents(const AnOwner : TComponent);
  var
    ComponentIndex : integer;
    StringListIndex : integer;
  begin
    for ComponentIndex := 0 to AnOwner.ComponentCount -1 do
    begin
      // update progressbar
      if not (AnOwner is TFrame) then
      begin
        frmExportProgress.ProgressBar1.StepIt;
      end;
      if not (AnOwner.Components[ComponentIndex].Name = '') then
      begin
        // process labels boxes
        if (AnOwner.Components[ComponentIndex] is TLabel) then
        begin
          ALabel := TLabel(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(ALabel) + ALabel.Name + '@';
...
//          end;
        ...
        // process edit boxes
        else if (AnOwner.Components[ComponentIndex] is TEdit) then
        begin
          AnEdit := TEdit(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(AnEdit) + AnEdit.Name + '@';
          if not (AnEdit = VersionControl) then
          begin
            for StringListIndex := 0 to Template.Count - 1 do
            begin
        ...
        else if (AnOwner.Components[ComponentIndex] is TJvFilenameEdit) then
        begin
          AFileEdit := TJvFilenameEdit(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(AnEdit) + AFileEdit.Name + '@';
          if not (AFileEdit = VersionControl) then
          begin
            for StringListIndex := 0 to Template.Count - 1 do
            begin
              AString := Template.Strings[StringListIndex];
              ...
              while Start > 0 do
              begin
                AString := Copy(AString, 1, Start -1) + AFileEdit.FileName
                  + Copy(AString, Start + Length(CurrentName),
                    Length(AString));
        ...
        else if (AnOwner.Components[ComponentIndex] is TArgusDataEntry) then
        begin
          AnArgusDataEntry
            := TArgusDataEntry(AnOwner.Components[ComponentIndex]);

          CurrentName := '@' + Prefix(AnArgusDataEntry) + AnArgusDataEntry.Name + '@';
        ...
        // process Radiogroups
        else if (AnOwner.Components[ComponentIndex] is TRadioGroup) then
        begin
          ARadioGroup := TRadioGroup(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(ARadioGroup) + ARadioGroup.Name + '@';
...
{$IFDEF FusedControls}
        else if (AnOwner.Components[ComponentIndex] is TFusedRadioGroup) then
        begin
          AFusedRadioGroup := TFusedRadioGroup(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(ARadioGroup) + AFusedRadioGroup.Name + '@';
          if not (AFusedRadioGroup = VersionControl) then
          begin
            for StringListIndex := 0 to Template.Count - 1 do
            begin
              AString := Template.Strings[StringListIndex];
              ...
              begin
                AString := Copy(AString, 1, Start -1)
                  + IntToStr(AFusedRadioGroup.ItemIndex) + Copy(AString, Start
                  + Length(CurrentName), Length(AString));
...
{$ENDIF}
        // process check boxes
        else if (AnOwner.Components[ComponentIndex] is TCheckBox) then
        begin
          ACheckBox := TCheckBox(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(ACheckBox) + ACheckBox.Name + '@';
        ...
        // process comboboxes
        else if (AnOwner.Components[ComponentIndex] is TComboBox) and
                 not ((AnOwner.Components[ComponentIndex] is TArgusDataEntry)) then
        begin
          AComboBox := TComboBox(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(AComboBox) + AComboBox.Name + '@';
        ...
        else if (AnOwner.Components[ComponentIndex] is TRadioButton) then
        begin
          ARadioButton := TRadioButton(AnOwner.Components[ComponentIndex]);
          CurrentName := '@' + Prefix(ARadioButton) + ARadioButton.Name + '@';
        ...
        else if (AnOwner.Components[ComponentIndex] is TFrame) then
        begin
          ProcessComponents(AnOwner.Components[ComponentIndex]);
        end;
      end;
    end;
  end;
...
function TArgusForm.FileVersion(const FileName: string): string;
var
...
begin
  ...
  begin
    AComponent := GetNextComponent(LineIndex, DataToRead,UnreadData,
      VersionControl, AFrame, FirstList, IgnoreList);
...
procedure TArgusForm.WriteJvFilenameEdit(ComponentData : TStringList; AComponent : TComponent);
begin
  ComponentData.Add(AComponent.Name);
  ComponentData.Add
    (TJvFilenameEdit(AComponent).Filename);
end;
...
procedure TArgusForm.ReadJvFilenameEdit(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  AFilenameEdit : TJvFilenameEdit;
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  AFilenameEdit := AComponent as TJvFilenameEdit;
  if not (AFilenameEdit = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(AComponent.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(AComponent.Name) = -1))
  then
  begin
    if Assigned(AFilenameEdit.OnEnter) then
    begin
      AFilenameEdit.OnEnter(AFilenameEdit);
    end;
    AFilenameEdit.FileName := AText;
    if Assigned(AFilenameEdit.OnChange) then
    begin
      AFilenameEdit.OnChange(AFilenameEdit);
    end;
    if Assigned(AFilenameEdit.OnExit) then
    begin
      AFilenameEdit.OnExit(AFilenameEdit);
    end;
  end;

end;

procedure TArgusForm.ReadCheckListBox(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
...
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  CheckListBox := AComponent as TCheckListBox;
  ReadData := not (CheckListBox = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(CheckListBox.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(CheckListBox.Name) = -1));
  if ReadData then
  begin
    if Assigned(CheckListBox.OnEnter) then
    begin
      CheckListBox.OnEnter(CheckListBox);
    end;
  end;
  Count := StrToInt(AText);
  for Index := 0 to Count -1 do
  begin
    if Index >= CheckListBox.Items.Count then
    begin
      Inc(LineIndex);
      continue;
    end;
    ...
    begin
      AText := DataToRead[LineIndex];
      CheckListBox.State[Index] := TCheckBoxState(StrToInt(AText));

      if Assigned(CheckListBox.OnClickCheck) then
      begin
        CheckListBox.OnClickCheck(CheckListBox);
      end;
    end;
    Inc(LineIndex);
  end;
  if ReadData then
  begin
    if Assigned(CheckListBox.OnExit) then
    begin
      CheckListBox.OnExit(CheckListBox);
    end;
  end;
end;

{$IFDEF VirtualStringTreeView}
procedure TArgusForm.ReadVirtualStringTree(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
...
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  VirtualStringTree := AComponent as TVirtualStringTree;
  ReadData := not (VirtualStringTree = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(VirtualStringTree.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(VirtualStringTree.Name) = -1));
  if ReadData then
  begin
    if Assigned(VirtualStringTree.OnEnter) then
    begin
      VirtualStringTree.OnEnter(VirtualStringTree);
    end;
  end;
  Count := StrToInt(AText);
  ANode := nil;
  ...
  begin
    ...
    begin
      ...
      if Index = 0 then
      begin
        ANode := VirtualStringTree.GetFirst;
      end
      else
      begin
        ANode := VirtualStringTree.GetNext(ANode);
      end;
      Assert(ANode <> nil);

      VirtualStringTree.CheckState[ANode] := TCheckState(StrToInt(AText));
  ...
  if ReadData then
  begin
    if Assigned(VirtualStringTree.OnExit) then
    begin
      VirtualStringTree.OnExit(VirtualStringTree);
    end;
  end;
end;

procedure TArgusForm.WriteVirtualStringTree(ComponentData: TStringList;
  AComponent: TComponent);
var
  VirtualStringTree: TVirtualStringTree;
  Node: PVirtualNode;
...
begin
  ...
  ComponentData.Add(AComponent.Name);
  Count := 0;
  Node := VirtualStringTree.GetFirst;
  while Node <> nil do
  begin
    Inc(Count);
    Node := VirtualStringTree.GetNext(Node);
  end;
  ...
  while Node <> nil do
  begin
    ComponentData.Add(IntToStr(Ord(VirtualStringTree.CheckState[Node])));
    Node := VirtualStringTree.GetNext(Node);
  end;
end;
{$ENDIF}
...
{$IFDEF RzTree}
procedure TArgusForm.ReadRzCheckTree(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent;
  DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  ARzCheckTree : TRzCheckTree;
  Count : integer;
  Index : integer;
  ReadData : boolean;
...
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  ARzCheckTree := AComponent as TRzCheckTree;
  ReadData := not (ARzCheckTree = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(ARzCheckTree.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(ARzCheckTree.Name) = -1));
  if ReadData then
  begin
    if Assigned(ARzCheckTree.OnEnter) then
    begin
      ARzCheckTree.OnEnter(ARzCheckTree);
    end;
  end;
  Count := StrToInt(AText);
  if ReadData then
  begin
    if ARzCheckTree.Items.Count > Count then
    begin
      ANode := ARzCheckTree.Items.Item[Count];
      while ANode <> nil do
      begin
        NextNode := ANode.getNextSibling;
        ARzCheckTree.Items.Delete(ANode);
        ANode := NextNode;
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      AText := DataToRead[LineIndex];
      if Index < ARzCheckTree.Items.Count then
      begin
        if AText = '0' then
        begin
          ARzCheckTree.ItemState[Index] := csUnknown;
        end
        else if AText = '1' then
        begin
          ARzCheckTree.ItemState[Index] := csUnchecked;
        end
        else if AText = '2' then
        begin
          ARzCheckTree.ItemState[Index] := csChecked;
        end
        else if AText = '3' then
        begin
          ARzCheckTree.ItemState[Index] := csPartiallyChecked;
        end;
      end;
    end;
    Inc(LineIndex);
  ...
  if ReadData then
  begin
    if Assigned(ARzCheckTree.OnExit) then
    begin
      ARzCheckTree.OnExit(ARzCheckTree);
    end;
  end;
end;
  {$ENDIF}
...
{$IFDEF FusedControls}
procedure TArgusForm.ReadFusedRadioGroup(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  ARadioGroup : TFusedRadioGroup;
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  ARadioGroup := AComponent as TFusedRadioGroup;
  if not (ARadioGroup = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(AComponent.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(AComponent.Name) = -1))
  then
...
procedure TArgusForm.ReadRbwDataGrid2(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  ...
  CanSelect : boolean;
begin
  Grid := AComponent as TRbwDataGrid2;

  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  RowLimit := StrToInt(AText);
  if (AComponent <> nil) and (Grid <> VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(AComponent.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(AComponent.Name) = -1))
  then
  begin
    if Assigned(Grid.OnEnter) then
    begin
      Grid.OnEnter(Grid);
    end;
    if SetRows(Grid) then
    begin
      Grid.RowCount := RowLimit;
    end;
  end;
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  ...
  then
  begin
    if SetColumns(Grid) then
    begin
      Grid.ColCount := ColLimit;
    end;
  end;
  For RowIndex := 0 to RowLimit -1 do
  begin
    ...
    begin
      ...
      begin
        ...
        if Grid.Columns[ColIndex].Format = rcf2Boolean then
        begin
          if AText = '1' then
          begin
            Grid.Checked[ColIndex,RowIndex] := True;
          end
          else if (AText = '0') or (AText = '') then
          begin
            Grid.Checked[ColIndex,RowIndex] := False;
          end
          else Assert((ColIndex < Grid.FixedCols)
            or (RowIndex < Grid.FixedRows));
        end
        else
        begin
          Grid.Cells[ColIndex,RowIndex] := AText;
          if Assigned(Grid.OnSetEditText) then
          begin
            Grid.OnSetEditText(Grid,ColIndex,RowIndex,AText);
          end;
        end;
      end;
    end;
...
procedure TArgusForm.ReadRbwDataGrid4(var LineIndex: integer;
  FirstList, IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  ...
  CanSelect : boolean;
begin
  Grid := AComponent as TRbwDataGrid4;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Grid.CheckState[ColIndex,RowIndex] := cbGrayed;
          end
          else Assert((ColIndex < Grid.FixedCols)
            or (RowIndex < Grid.FixedRows));
        end
...
procedure TArgusForm.WriteCheckListBox(ComponentData: TStringList;
  AComponent: TComponent);
var
  CheckListBox : TCheckListBox;
  Index : integer;
begin
  CheckListBox := AComponent as TCheckListBox;
  ComponentData.Add(AComponent.Name);
  ComponentData.Add(IntToStr(CheckListBox.Items.Count));
  for Index := 0 to CheckListBox.Items.Count -1 do
  begin
    ComponentData.Add(IntToStr(Ord(CheckListBox.State[Index])));
  end;
end;


  {$IFDEF RzTree}
procedure TArgusForm.WriteRzCheckTree(ComponentData: TStringList;
  AComponent: TComponent);
var
  ARzCheckTree : TRzCheckTree;
  Index : integer;
begin
  ARzCheckTree := AComponent as TRzCheckTree;
  ComponentData.Add(AComponent.Name);
  ComponentData.Add(IntToStr(ARzCheckTree.Items.Count));
  for Index := 0 to ARzCheckTree.Items.Count -1 do
  begin
    ComponentData.Add(IntToStr(Ord(ARzCheckTree.ItemState[Index])));
  end;
end;
  {$ENDIF}
...
begin
  ...
  {$IFDEF FusedControls}
procedure TArgusForm.WriteFusedRadioGroup(ComponentData: TStringList;
  AComponent: TComponent);
begin
  ComponentData.Add(AComponent.Name);
  ComponentData.Add
    (IntToStr(TFusedRadioGroup(AComponent).ItemIndex));
end;
  {$ENDIF}
...
procedure TArgusForm.WriteRbwDataGrid2(ComponentData: TStringList;
  AComponent: TComponent);
var
  RowIndex,ColIndex : integer;
  Grid: TRbwDataGrid2;
begin
  Grid := AComponent as TRbwDataGrid2;
  ComponentData.Add(Grid.Name);
  ComponentData.Add
    (IntToStr(Grid.RowCount));
  ComponentData.Add
    (IntToStr(Grid.ColCount));
  For RowIndex := 0 to
    Grid.RowCount -1 do
  begin
    ...
    begin
      if Grid.Columns[ColIndex].Format = rcf2Boolean then
      begin
        if Grid.Checked[ColIndex,RowIndex] then
        begin
          ComponentData.Add('1');
        end
        else
      ...
      begin
        ComponentData.Add(Grid.Cells[ColIndex,RowIndex]);
      end;
    end;
  end;
end;

procedure TArgusForm.WriteRbwDataGrid4(ComponentData: TStringList;
  AComponent: TComponent);
var
  RowIndex,ColIndex : integer;
  Grid: TRbwDataGrid4;
begin
  ...
  begin
    ...
    begin
      if Grid.Columns[ColIndex].Format = rcf4Boolean then
      begin
        ComponentData.Add(IntToStr(Ord(Grid.CheckState[ColIndex, RowIndex])));
      end
      else
      begin
        ComponentData.Add(Grid.Cells[ColIndex,RowIndex]);
...
begin
  ...
  else if AComponent is TJvFilenameEdit then
  begin
    WriteJvFilenameEdit(ComponentData, AComponent);
    result := 1;
  end
  ...
  {$IFDEF FusedControls}
  // add data for fused radio groups.
  else if AComponent is TFusedRadioGroup then
  begin
    WriteFusedRadioGroup(ComponentData, AComponent);
    result := 1;
  end
  {$ENDIF}
  ...
  // add data for TRbwDataGrid2.
  else if AComponent is TRbwDataGrid2 then
  begin
    WriteRbwDataGrid2(ComponentData, AComponent);
    result := 1;
  end
  else if AComponent is TRbwDataGrid4 then
  begin
    WriteRbwDataGrid4(ComponentData, AComponent);
    result := 1;
  end
  ...
  {$IFDEF RzTree}
  else if AComponent is TRzCheckTree then
  begin
    WriteRzCheckTree(ComponentData, AComponent);
    result := 1;
  end
  {$ENDIF}
  else if AComponent is TCheckListBox then
  begin
    WriteCheckListBox(ComponentData, AComponent);
    result := 1;
  end
{$IFDEF StringGrid3d}
...
{$ENDIF}
  ...
  begin
    WriteVirtualStringTree(ComponentData, AComponent);
    result := 1;
  end
  {$ENDIF}
  {$IFDEF RbwCheckTreeView}
  else if AComponent is TRbwCheckTreeView then
  begin
    WriteRbwCheckTreeView(ComponentData, AComponent);
    result := 1;
  end
  {$ENDIF}
  ...
  begin
    WriteTreeView(ComponentData, AComponent);
    result := 1;
  end
end;
...
begin
  ...
  begin
    ...
    // If you can't find the compenent, add its name to UnreadData
...
begin
  ...
  begin
    ReadJvFileNameEdit(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
  // read data for TArgusDataEntry's
  else if AComponent is TArgusDataEntry then
  ...
  begin
    ReadFusedRadioGroup(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
  {$ENDIF}
  ...
  // read data for TRbwDataGrid2
  else if (AComponent is TRbwDataGrid2) then
  begin
    ReadRbwDataGrid2(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
  else if (AComponent is TRbwDataGrid4) then
  begin
    ReadRbwDataGrid4(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
...
{$IFDEF RzTree}
  ...
  begin
    ReadRzCheckTree(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
{$ENDIF}
  else if (AComponent is TCheckListBox) then
  begin
    ReadCheckListBox(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
{$IFDEF DataTableInstalled}
  // obsolete
  ...
  begin
    ...
    if not (ADatatbl = VersionControl) and
      ((FirstList = nil) or (FirstList.IndexOf(AComponent.Name)>-1)) then
    begin
      ...
      begin
        ...
        begin
          ...
          if not (ADatatbl = VersionControl) and
            ((FirstList = nil) or (FirstList.IndexOf(AComponent.Name)>-1)) then
          begin
            ADatatbl.CellSet.Item[RowIndex, ColIndex].Value := InternationalStrToFloat(AText);
          end;
        end;
      end
...
{$ENDIF}
  ...
  begin
    ReadVirtualStringTree(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
  {$ENDIF}
  {$IFDEF RbwCheckTreeView}
  ...
  begin
    ReadRbwCheckTreeView(LineIndex, FirstList, IgnoreList, AComponent, DataToRead,
      VersionControl);
  end
  {$ENDIF}
  else if (AComponent is TFrame) then
  ...
  begin
    ReadTreeView(LineIndex, FirstList, IgnoreList, AComponent,
      DataToRead, VersionControl);
  end;
end;
...
constructor TArgusForm.Create(AOwner: TComponent);
begin
  WriteDebugOutput('begin TArgusForm.Create');
  inherited;
  WriteDebugOutput('end TArgusForm.Create');

end;
    {$ENDIF}

procedure TArgusForm.AssignHelpFile(FileName : string) ;
{$IFNDEF HTMLHELP}
var
    DllDirectory : String;
{$ENDIF}
...
procedure TArgusForm.SetControlColor(const AControl: TControl);
begin
  // set the color of controls when they change from enabled to disabled
  // or back.
  if AControl.Enabled then
  begin
    TControlCrack(AControl).Color := clWindow;
  end
  else
  begin
    TControlCrack(AControl).Color := clBtnFace;
  end;
end;

procedure TArgusForm.ReadTreeView(var LineIndex: integer; FirstList,
  IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
...
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  TreeView := AComponent as TTreeView;
  ReadData := not (TreeView = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(TreeView.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(TreeView.Name) = -1));
  if ReadData then
  begin
    if Assigned(TreeView.OnEnter) then
    begin
      TreeView.OnEnter(TreeView);
    end;
  ...
  begin
    ...
    begin
      AText := DataToRead[LineIndex];
      TreeView.Items.Add(nil, AText);
    end;
    Inc(LineIndex);
  end;
  if ReadData then
  begin
    if Assigned(TreeView.OnExit) then
    begin
      TreeView.OnExit(TreeView);
    end;
  end;
end;

procedure TArgusForm.WriteTreeView(ComponentData: TStringList;
  AComponent: TComponent);
var
  TreeView: TTreeView;
  Node: TTreeNode;
...
begin
  ...
  while Node <> nil do
  begin
    ComponentData.Add(Node.Text);
    Node := Node.GetNext;
  end;
end;

{$IFDEF RbwCheckTreeView}
procedure TArgusForm.ReadRbwCheckTreeView(var LineIndex: integer; FirstList,
  IgnoreList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  RbwCheckTreeView: TRbwCheckTreeView;
  Count : integer;
  Index : integer;
  ReadData : boolean;
  ANode : TTreeNode;
begin
  AText := DataToRead[LineIndex];
  Inc(LineIndex);
  RbwCheckTreeView := AComponent as TRbwCheckTreeView;
  ReadData := not (RbwCheckTreeView = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(RbwCheckTreeView.Name) > -1)) and
    ((IgnoreList = nil) or (IgnoreList.IndexOf(RbwCheckTreeView.Name) = -1));
  if ReadData then
  begin
    if Assigned(RbwCheckTreeView.OnEnter) then
    begin
      RbwCheckTreeView.OnEnter(RbwCheckTreeView);
    end;
  end;
  Count := StrToInt(AText);
  ANode := nil;
  ...
  begin
    ...
    begin
      ...
      if Index = 0 then
      begin
        ANode := RbwCheckTreeView.Items.GetFirstNode;
      end
      else
      begin
        if ANode <> nil then
      ...
      if ANode <> nil then
      begin
        ANode.StateIndex := StrToInt(AText);
      end;
    end;
    Inc(LineIndex);
  end;
  if ReadData then
  begin
    if Assigned(RbwCheckTreeView.OnExit) then
    begin
      RbwCheckTreeView.OnExit(RbwCheckTreeView);
    end;
  end;
end;

procedure TArgusForm.WriteRbwCheckTreeView(ComponentData: TStringList;
  AComponent: TComponent);
var
  RbwCheckTreeView: TRbwCheckTreeView;
  Node: TTreeNode;
...
begin
  ...
  ComponentData.Add(AComponent.Name);
  Count := 0;
  Node := RbwCheckTreeView.Items.GetFirstNode;
  while Node <> nil do
  begin
    Inc(Count);
    Node := Node.GetNext;
  ...
  while Node <> nil do
  begin
    ComponentData.Add(IntToStr(Node.StateIndex));
    Node := Node.GetNext;
  end;
end;
{$ENDIF}
