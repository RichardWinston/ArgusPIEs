...
begin
  ...
  then
    ...
    //  maplayerhandle := nil;
      ...
      maplayerhandle := GetLayerHandle(ModelHandle,MapLayerName);
...
//        PChar(MapLayerName) );
      ...
      then
        begin
          dataLayerHandle := GetLayerHandle(ModelHandle,dataLayerName);
...
//            PChar(dataLayerName) );
    ...
    //        A_MapsLayer.Name := MapLayerName;
     ...
     then
       begin
         Beep;
         MessageDlg('Error creating postprocessing layer', mtError,[mbOk],0);
       end
     else
       begin
         try
            GetMem(ANE_String, Length(StringToWrite) + 1);
            try
              StrPCopy(ANE_String, StringToWrite);
           ...
           begin
             Beep;
             MessageDlg('Unable to create chart. This error may occur if all'
               + ' the points are in a straight line.', mtError,[mbOk],0);
           end;
         end;
       end;
