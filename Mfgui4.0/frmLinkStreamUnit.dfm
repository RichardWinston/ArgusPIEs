inherited frmLinkStreams: TfrmLinkStreams
  Width = 551
  Height = 450
  HelpContext = 11231
  Caption = 'Link Streams'
  Font.Height = -18
  Font.Name = 'Times New Roman'
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 20
  object zbStreams: TRbwZoomBox
    Left = 0
    Top = 0
    Width = 543
    Height = 356
    HorzScrollBar.Range = 539
    VertScrollBar.Range = 352
    Align = alClient
    AutoScroll = False
    Color = clWindow
    ParentColor = False
    TabOrder = 0
    OnMouseDown = zbStreamsMouseDown
    OnMouseMove = zbStreamsMouseMove
    OnMouseUp = zbStreamsMouseUp
    BottomMargin = 20
    LeftMargin = 20
    RightMargin = 20
    TopMargin = 20
    MinX = 20
    MaxX = 519
    MinY = 20
    MaxY = 332
    Multiplier = 1
    OnPaint = zbStreamsPaint
    PBColor = clWindow
    PBCursor = crDefault
    PBDragCursor = crDrag
    PBDragKind = dkDrag
    PBDragMode = dmManual
    PBEnabled = True
    PBFont.Charset = ANSI_CHARSET
    PBFont.Color = clWindowText
    PBFont.Height = -18
    PBFont.Name = 'Times New Roman'
    PBFont.Style = []
    PBHeight = 352
    PBLeft = 0
    PBShowHint = False
    PBTag = 0
    PBTop = 0
    PBVisible = True
    PBWidth = 539
    SBrush.Style = bsClear
    SCursor = crDefault
    SDragCursor = crDrag
    SelectionWidth = 3
  end
  object Panel1: TPanel
    Left = 0
    Top = 356
    Width = 543
    Height = 41
    Align = alBottom
    TabOrder = 1
    object sbPick: TSpeedButton
      Left = 8
      Top = 8
      Width = 23
      Height = 22
      Hint = 'Select streams'
      GroupIndex = 1
      Down = True
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbPickClick
    end
    object sbZoomIn: TSpeedButton
      Left = 32
      Top = 8
      Width = 23
      Height = 22
      Hint = 'Zoom in'
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbZoomInClick
    end
    object sbZoomOut: TSpeedButton
      Left = 56
      Top = 8
      Width = 23
      Height = 22
      Hint = 'Zoom out'
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbZoomOutClick
    end
    object sbZoom: TSpeedButton
      Left = 80
      Top = 8
      Width = 23
      Height = 22
      Hint = 'Zoom in on a selected area'
      GroupIndex = 1
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbZoomClick
    end
    object sbZoomExtents: TSpeedButton
      Left = 102
      Top = 8
      Width = 22
      Height = 22
      Hint = 'Zoom out all the way'
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbZoomExtentsClick
    end
    object btnCancel: TBitBtn
      Left = 448
      Top = 8
      Width = 83
      Height = 25
      TabOrder = 0
      Kind = bkCancel
    end
    object btnOK: TBitBtn
      Left = 352
      Top = 8
      Width = 89
      Height = 25
      TabOrder = 1
      OnClick = btnOKClick
      Kind = bkOK
    end
    object btnLinkStreams: TButton
      Left = 136
      Top = 8
      Width = 105
      Height = 25
      Caption = 'Link Streams'
      TabOrder = 2
      OnClick = btnLinkStreamsClick
    end
    object btnHelp: TBitBtn
      Left = 264
      Top = 8
      Width = 83
      Height = 25
      TabOrder = 3
      Kind = bkHelp
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 397
    Width = 543
    Height = 19
    Panels = <
      item
        Width = 271
      end
      item
        Width = 50
      end>
    SimplePanel = False
  end
  object qtStartPoints: TRbwQuadTree
    MaxPoints = 100
    Left = 42
    Top = 42
  end
  object qtSelectedStartPoints: TRbwQuadTree
    MaxPoints = 100
    Left = 82
    Top = 42
  end
end
