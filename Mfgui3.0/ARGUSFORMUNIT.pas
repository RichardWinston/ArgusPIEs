...
type
  ...
  TSpecialHandling = procedure(var LineIndex : integer; FirstList: TStringlist;
  ...
  published
    procedure ReadEdit(var LineIndex: integer; FirstList: TStringlist;
    ...
    procedure ReadArgusDataEntry(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an ArgusDataEntry and call its OnEnter, OnChange and OnExit events.
    procedure ReadRadioGroup(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an radiogroup and call its OnEnter, OnClick and OnExit events.
    procedure ReadComboBox(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an combobox and call its OnEnter, OnChange and OnExit events.
    procedure ReadCheckBox(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an checkbox and call its OnEnter, OnClick and OnExit events.
    procedure ReadRadioButton(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an radiobutton and call its OnEnter, OnClick and OnExit events.
    procedure ReadStringGrid(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an stringgrid and call its OnEnter, OnSelectCell,
    // OnSetEditText and OnExit events.
    procedure ReadMemo(var LineIndex: integer; FirstList: TStringlist;
      AComponent: TComponent; DataToRead: TStringList;
      const VersionControl : TControl); virtual;
    // read data for an combobox and call its OnEnter, OnChange and OnExit events.
  ...
  public
    ...
    procedure StringToForm(const DataFromArgus : String; UnreadData : TStringlist;
       ...
       const ShowError : boolean; FirstList: TStringlist;
...
procedure TArgusForm.StringToForm(const DataFromArgus : String; UnreadData : TStringlist;
       const VersionControl : TControl; var VersionInString : string;
       const ShowError : boolean; FirstList: TStringlist; var Developer : String);
var
  {$IFDEF DataTableInstalled}
  ADatatbl : TDatatbl;
  {$ENDIF}
    ...
    begin
      try // try 2 : try except
        begin
          ...
          begin
            // read the vrsion information from the data and compare it to
            ...
            begin
              ...
              begin
                ...
                ASpecialHandlingObject.ReadData(LineIndex, FirstList,
...
{$ENDIF}
                // read data for memo's
                  ...
                  begin
                    ReadMemo(LineIndex, FirstList, AComponent, DataToRead,
                      VersionControl);
                  end
              end;
            end;
...
var
  ...
  RowIndex, ColIndex, MemoIndex : integer; // indicies for loop over rows
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if Components[ComponentIndex] is TStringGrid then
            begin
              ComponentData.Add(AComponent.Name);
              ComponentData.Add
  ...
  {$IFDEF DataTableInstalled}
          ...
          else if Components[ComponentIndex] is TDataTbl then
            begin
              ComponentData.Add(AComponent.Name);
              ComponentData.Add
  ...
  {$ENDIF}
          // add data for memos
          else if Components[ComponentIndex] is TMemo then
            begin
              ComponentData.Add(Components[ComponentIndex].Name);
              ComponentData.Add
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // process edit boxes
        if (Components[ComponentIndex] is TEdit) then
        begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              while Start > 0 do
              begin
                AString := Copy(AString, 1, Start -1) + AnArgusDataEntry.Text
                  + Copy(AString, Start + Length(CurrentName),
                    Length(AString));
...
procedure TArgusForm.ReadEdit(var LineIndex : integer; FirstList : TStringlist;
...
procedure TArgusForm.ReadArgusDataEntry(var LineIndex: integer;
  FirstList: TStringlist; AComponent: TComponent; DataToRead: TStringList;
  const VersionControl: TControl);
var
  AText : String;
  AnArgusDataEntry : TArgusDataEntry;
...
begin
  ...
  Inc(LineIndex);
  RowLimit := StrToInt(AText);
  if not (AStringGrid = VersionControl) and
    ((FirstList = nil) or (FirstList.IndexOf(AComponent.Name)>-1))
  then
  begin
...
begin
  ...
  // read all other data.
  StringToForm(DataToRead, UnreadData,
       VersionControl, VersionInString, True, nil,
       PIEDeveloper);
