...
uses Variables, ANE_LayerUnit, ArgusFormUnit, ANECB, UtilityFunctions,
  ModflowUnit, ExportProgressUnit, RunUnit;
      ...
      begin
        ...
        begin
          WellLayerName := ModflowTypes.GetMFWellLayerType.ANE_LayerName
            + IntToStr(UnitIndex);
          CurrentLayerHandle := GetLayerHandle(frmMODFLOW.CurrentModelHandle,WellLayerName);
{          GetMem(An_ANE_STR, Length(WellLayerName) + 1);
          try
            StrPCopy(An_ANE_STR,WellLayerName);
          ...
          begin
            ...
            GetMem(AString, Length(StringToImport) + 1);
            try
              StrPCopy(AString, StringToImport);
              ANE_ImportTextToLayerByHandle(frmMODFLOW.CurrentModelHandle,
                CurrentLayerHandle, AString);
            finally
              FreeMem(AString);
            end;
          end;
...
{        GetMem(An_ANE_STR, Length(WellLayerName) + 1);
        ...
        try
          StrPCopy(AString, StringToImport);
          ANE_ImportTextToLayerByHandle(frmMODFLOW.CurrentModelHandle,
            frmWellData.CurrentLayerHandle, AString);
        finally
          FreeMem(AString);
        end;
      end;
