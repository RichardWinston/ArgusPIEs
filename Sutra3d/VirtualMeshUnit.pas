...
{    function ParameterIndex(ModelHandle : ANE_PTR; ParameterName : string) : ANE_INT16;
  ...
  public
    ...
    function IsNa(ModelHandle: ANE_PTR; X, Y: Double;
...
//    function GetLayerHandle(ModelHandle : ANE_PTR; LayerIndex : integer) : ANE_PTR; virtual;
  ...
  // variables are set in TVirtual3DMesh.MakeVirtualNodes
    ...
    public
      ...
      function MorphedIsNA(ModelHandle : ANE_PTR; Expression : string) : ANE_BOOL;
...
{    procedure GetMorphedFraction(LayerAbove, LayerBelow: TReal2dMesh;
  ...
  Public
    ...
    procedure ParseIsNaExpression(ModelHandle: ANE_PTR; Expression: string;
...
function TReal2DMesh.IsNa(ModelHandle : ANE_PTR; X,Y : Double; Expression: String): ANE_BOOL;
var
  LayerOptions : TLayerOptions;
  LayerIndexString : String;
begin
  ...
  Expression := 'IsNA(' + Expression + LayerIndexString + ')';
  LayerOptions := OptionsList.Objects[OptionsList.IndexOf(Expression)] as TLayerOptions;
  result := LayerOptions.BooleanValueAtXY(ModelHandle, X,Y{,Expression});
end;
...
procedure TVirtual3DMesh.ParseIsNaExpression(ModelHandle: ANE_PTR; Expression: string;
  NumberType : TPIENumberType);
var
  Index : integer;
  AMesh : TReal2DMesh;
...
begin
  ...
  begin
    ...
    AMesh.ParseExpression(ModelHandle, 'IsNA('+ Expression + LayerIndex + ')',NumberType);
  end;
end;
...
function TVirtualNode.MorphedIsNA(ModelHandle: ANE_PTR;
  Expression: string): ANE_BOOL;
begin
//  Expression := 'IsNA(' + Expression + ')';
  if MeshAbove <> nil then
  begin
    result := MeshAbove.IsNA(ModelHandle, AboveEvalLocation.X,
  ...
  begin
    ...
    begin
      result := NodeAbove.FMesh.IsNA(ModelHandle, AboveEvalLocation.X,
        AboveEvalLocation.Y,Expression);
    end
    else
    begin
      result := NodeAbove.FMesh.IsNA(ModelHandle, AboveEvalLocation.X,
        AboveEvalLocation.Y,Expression)
        or NodeBelow.FMesh.IsNA(ModelHandle, BelowEvalLocation.X,
