...
//  PLayerRecord = ^TLayerRecord;
  ...
  TfrmSutra = class(TArgusForm)
    ...
    Label134: TLabel;
    cbSutra3: TCheckBox;
    rgDensityFunction: TRadioGroup;
    cbPressureOverride: TCheckBox;
    cbConcentrationOverride: TCheckBox;
    ...
    procedure rdgFreezingEqSetEditText(Sender: TObject; ACol,
      ARow: Integer; const Value: String);
    procedure rctreeBoundariesExit(Sender: TObject);
    procedure cbUseRhoFormulaClick(Sender: TObject);
    procedure rgDensityFunctionClick(Sender: TObject);
    procedure cbPressureOverrideClick(Sender: TObject);
    procedure cbConcentrationOverrideClick(Sender: TObject);
  private
    ...
    procedure AllowPressureOverride;
    procedure AllowConcentrationOverride;
  ...
  public
    ...
    procedure SetTimeDepExpressions;
...
{$ENDIF}
    ...
    function ParseInitialPressureOverride: boolean;
    function ParseInitialConcentration: boolean;
    function ParseInitialConcentrationOverride: boolean;
...
begin
  ...
  SutraLayerStructure.UnIndexedLayers0.SetUnIndexedExpressionNow
    (TSutraMeshLayer, TPVEC_OverrideParam, TPVEC_OverrideParam.ANE_ParamName, True);
...
begin
  ...
  SutraLayerStructure.UnIndexedLayers0.SetUnIndexedExpressionNow
    (TSutraMeshLayer, TUVEC_OverrideParam, TUVEC_OverrideParam.ANE_ParamName, True);
...
procedure TfrmSutra.SetTimeDepExpressions;
begin
  SutraLayerStructure.UnIndexedLayers0.SetUnIndexedExpressionNow
    (TSutraMeshLayer, TTimeDepFluidSourceParam, TTimeDepFluidSourceParam.ANE_ParamName, True);

  SutraLayerStructure.UnIndexedLayers0.SetUnIndexedExpressionNow
    (TSutraMeshLayer, TTimeDepEnergySoluteSourceParam, TTimeDepEnergySoluteSourceParam.ANE_ParamName, True);

  SutraLayerStructure.UnIndexedLayers0.SetUnIndexedExpressionNow
    (TSutraMeshLayer, TTimeDepSpecHeadPresParam, TTimeDepSpecHeadPresParam.ANE_ParamName, True);

  SutraLayerStructure.UnIndexedLayers0.SetUnIndexedExpressionNow
    (TSutraMeshLayer, TTimeDepSpecConcTempParam, TTimeDepSpecConcTempParam.ANE_ParamName, True);
  ...
  begin
    ...
    begin
      ...
      MeshLayer.AddOrRemoveUnIndexedParameter(TQINParam,
        not Is3D);
      MeshLayer.AddOrRemoveUnIndexedParameter(TQIN_CommentParam,
        not Is3D);
      ...
      MeshLayer.AddOrRemoveIndexedParameter1(TPVECParam,
        VerticallyAlignedMesh);
      MeshLayer.AddOrRemoveIndexedParameter1(TPVEC_OverrideParam,
        VerticallyAlignedMesh and cbPressureOverride.Enabled and cbPressureOverride.Checked);


      MeshLayer.AddOrRemoveIndexedParameter1(TUVECParam,
        VerticallyAlignedMesh);
      MeshLayer.AddOrRemoveIndexedParameter1(TUVEC_OverrideParam,
        VerticallyAlignedMesh and cbConcentrationOverride.Enabled and cbConcentrationOverride.Checked);
...
//  cbUseRhoFormula.Visible := True;
  rgDensityFunction.Visible := True;
{$ELSE}
  rbFreezing.Enabled := False;
//  cbUseRhoFormula.Visible := False;
  rgDensityFunction.Visible := False;
{$ENDIF}
...
//  cbUseRhoFormula.Enabled := EnergyUsed or rbGeneral.Checked;
//  if not cbUseRhoFormula.Enabled then
//  begin
//    cbUseRhoFormula.Checked := False;
//  end;
  rgDensityFunction.Enabled := EnergyUsed or rbGeneral.Checked;
  if not rgDensityFunction.Enabled then
  begin
    rgDensityFunction.ItemIndex := 0;
  end;
...
begin
  ...
  case TransportType of
    ...
    ttEnergy:
      begin
        if rgDensityFunction.ItemIndex = 1 then
        begin
          lblRHOW0_Desc.Caption := 'Density scaling factor';
        end
        else
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      LayerList.AddOrRemoveLayer(TInitialPressureOverrideLayer,
        cbPressureOverride.Enabled and cbPressureOverride.Checked);

      LayerList.AddOrRemoveLayer(TInitialConcTempLayer, True);
      LayerList.AddOrRemoveLayer(TInitialConcTempOverrideLayer,
        cbConcentrationOverride.Enabled and cbConcentrationOverride.Checked);
    ...
    begin
      ...
      ParamList.AddOrRemoveParameter(TPVECParam, TPVECParam.ANE_ParamName,
        Index <> Limit);
      ParamList.AddOrRemoveParameter(TPVEC_OverrideParam, TPVEC_OverrideParam.ANE_ParamName,
        (Index <> Limit) and cbPressureOverride.Enabled
        and cbPressureOverride.Checked);
      ParamList.AddOrRemoveParameter(TUVECParam, TUVECParam.ANE_ParamName,
        Index <> Limit);
      ParamList.AddOrRemoveParameter(TUVEC_OverrideParam, TUVEC_OverrideParam.ANE_ParamName,
        (Index <> Limit) and cbConcentrationOverride.Enabled and cbConcentrationOverride.Checked);
    end;
  end;
end;
...
function TfrmSutra.ParseInitialPressureOverride: boolean;
begin
  result := True;
  try
    if cbPressureOverride.Enabled and cbPressureOverride.Checked then
    begin
      VirtualMesh.ParseExpression(CurrentModelHandle, TPVEC_OverrideParam.WriteParamName,
        pnFloat);
      VirtualMesh.ParseIsNaExpression(CurrentModelHandle, TPVEC_OverrideParam.WriteParamName,
        pnBoolean);
    end;        
  except on E: Exception do
    begin
...
function TfrmSutra.ParseInitialConcentrationOverride: boolean;
begin
  result := True;
  try
    if cbConcentrationOverride.Enabled and cbConcentrationOverride.Checked then
    begin
      VirtualMesh.ParseExpression(CurrentModelHandle, TUVEC_OverrideParam.WriteParamName,
        pnFloat);
      VirtualMesh.ParseIsNaExpression(CurrentModelHandle, TUVEC_OverrideParam.WriteParamName,
        pnBoolean);
    end;        
  except on E: Exception do
    begin
...
begin
  ...
  AllowConcentrationOverride;
  AllowPressureOverride;
...
procedure TfrmSutra.AllowConcentrationOverride;
begin
  cbPressureOverride.Enabled := (rgDimensions.ItemIndex = 1) and
    ((rgInitialValues.ItemIndex in [1,3])
    or (rgInterpolateInitialValues.ItemIndex in [1,3]))
end;

procedure TfrmSutra.AllowPressureOverride;
begin
  cbConcentrationOverride.Enabled :=  (rgDimensions.ItemIndex = 1) and
    ((rgInitialValues.ItemIndex in [2,3])
    or (rgInterpolateInitialValues.ItemIndex in [2,3]))
end;
      ...
      case BoundaryType of
        btSourcesOfFluid:
          begin
            ...
            ParamList.AddOrRemoveParameter(TQIN_CommentParam,
              TQIN_CommentParam.ANE_ParamName, ShouldBePresent);
          ...
          btSourcesOfFluid:
            begin
              MeshLayer.ParamList.AddOrRemoveParameter(TQINParam,
                TQINParam.ANE_ParamName, ShouldBePresent);
              MeshLayer.ParamList.AddOrRemoveParameter(TQIN_CommentParam,
                TQIN_CommentParam.ANE_ParamName, ShouldBePresent);
...
procedure TfrmSutra.cbConcentrationOverrideClick(Sender: TObject);
begin
  inherited;
  SetSoluteEnergyExpressions;
  AddOrRemoveParameters;
...
begin
  ...
  begin
    if not Loading and not Cancelling then
    begin
      if lvChoices.Items[Ord(poInverse)] <> nil then
      begin
...
procedure TfrmSutra.cbPressureOverrideClick(Sender: TObject);
begin
  inherited;
  SetPressureHeadExpressions;
  AddOrRemoveParameters;
...
begin
  ...
  try
    ...
    begin
      {$IFDEF UseXY}
      UcodeParser.CreateVariable('X', '', 0.0, 'X');
      UcodeParser.CreateVariable('Y', '', 0.0, 'Y');
      ...
      begin
        ...
        begin
          VariableName := rdgFactorGrid.Cells[Ord(fcName), RowIndex];
          UcodeParser.CreateVariable(VariableName, '', 0.0, VariableName);
        end;
      end;
      {$ENDIF}
      Names := TStringList.Create;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              UcodeParser.CreateVariable(ParamName, '', Value, ParamName);
...
begin
  ...
  try
    ...
    frmEquationEditor.rbFormulaParser.CreateVariable('X',
      'Argus ONE Functions', 0.0, 'X');
    frmEquationEditor.rbFormulaParser.CreateVariable('Y',
      'Argus ONE Functions', 0.0, 'Y');
    for RowIndex := 1 to rdgFactorGrid.RowCount -1 do
    begin
      if (rdgFactorGrid.Cells[Ord(fcName),RowIndex] <> '')
        and (rdgFactorGrid.Cells[Ord(fcLayer),RowIndex] <> '') then
      begin
        ...
        frmEquationEditor.rbFormulaParser.CreateVariable(
          VariableName,
          'Argus ONE Factors', 0.0, VariableName);
      end;
...
begin
  ...
  try
    ...
    frmSutra.UcodeCompiler.CreateVariable('X', '', 0.0, 'X');
    frmSutra.UcodeCompiler.CreateVariable('Y', '', 0.0, 'Y');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Inc(NewLength);
          frmSutra.UcodeCompiler.CreateVariable(PName, '', 0.0, PName);
        end;
      end;
    end;
    ...
    begin
      frmSutra.XVar := TSubConstant.Create('X', 'X');
    end;
    if frmSutra.YVar = nil then
    begin
      frmSutra.YVar := TSubConstant.Create('Y', 'Y');
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SpecialVariable:= TSubConstant.Create(PName, PName);
...
procedure TfrmSutra.rctreeBoundariesExit(Sender: TObject);
var
  Node: TTreeNode;
begin
  inherited;
  if rctreeBoundaries.Items.Count > 0 then
  begin
    Node := rctreeBoundaries.Items[0];
    while Node <> nil do
    begin
      rctreeBoundariesChange(Sender, Node);
      Node := Node.GetNext;
    end;
  end;
end;

procedure TfrmSutra.cbUseRhoFormulaClick(Sender: TObject);
begin
  inherited;
  SetlblRHOW0_Desc_Caption;
  if not Loading and not Cancelling then
  ...
  if rgDensityFunction.ItemIndex = 1 then
  begin
    adeRHOW0.CheckMin := True;
  end
  else
  begin
    adeRHOW0.CheckMin := False;
  end;
  SetlblRHOW0_Desc_Caption;
  adeURHOW0.Enabled := not (rgDensityFunction.Enabled
    and (rgDensityFunction.ItemIndex = 1));
  adeDRWDU.Enabled := adeURHOW0.Enabled;
end;

procedure TfrmSutra.rgDensityFunctionClick(Sender: TObject);
begin
  inherited;
  SetlblRHOW0_Desc_Caption;
  if not Loading and not Cancelling then
