...
uses WinTypes, Classes, SysUtils, S_String;
...
uses WinProcs, Dialogs, Controls, S_Consts;
...
var
  ...
  Module: THandle;
begin
{ Find which module has the caller's resources. }
{$ifdef Delphi3}
  Module := FindResourceHInstance(FindHInstance(PChar(ResourceName)));
{$else}
  Module := hInstance;
{$endif}

  ResInfo := FindResource(Module,
                          StrToPChar(AnsiUpperCase(ResourceName)),
                          S_ResourceType);
  if ResInfo = 0 then
    raise Exception.CreateResFmt(S_NoSuchResource, [ResourceName]);

  fHandle := LoadResource(Module, ResInfo);
  if fHandle = 0 then
