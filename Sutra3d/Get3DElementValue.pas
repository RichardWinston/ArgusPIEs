...
procedure GetCoordinates(var X, Y , Z : double; var I : longint);
  stdcall; external 'SutraReader22.dll' name 'getcoordinates';

procedure GetNodeNumber(var NodeNum, IElem, INode : longint);
  stdcall; external 'SutraReader22.dll' name 'getnodenumber';

procedure INITIALIZE(var NumNodes, NumElem, NPresB, NConcB, IERRORCODE: longint;
  InputFile : string; FileNameLength : longint);
  stdcall; external 'SutraReader22.dll' name 'initialize';

procedure CLOSE_FILE;
  stdcall; external 'SutraReader22.dll' name 'close_file';
...
var
  MyFileName: string;
begin
  MyFileName := Copy(RestartFileName, 1, 1000000);
  ReadMeshFromFile(InputFileName);
  ReadRestartFile(MyFileName);
  StoreData;
end;
...
begin
  ...
  try
    SutraFile.LoadFromFile(InputFileName);
    LineIndex := SutraFile.IndexOf('# Data Set 2B');
    if (SutraFile.Count > 0) and (LineIndex >= 0) then
    begin
      Splitter.Delimiter := ' ';

      Assert(LineIndex >= 0);
      Splitter.DelimitedText := SutraFile[LineIndex + 1];
//      Splitter.DelimitedText := Splitter[0];
      ThreeD := UpperCase(Splitter[0]) = '''3D';

      LineIndex := SutraFile.IndexOf('# Data Set 3');
      Assert(LineIndex >= 0);
      Splitter.DelimitedText := SutraFile[LineIndex + 1];
      NumNodes := StrToInt(Splitter[0]);
      NumElem := StrToInt(Splitter[1]);

      LineIndex := SutraFile.IndexOf('#Start_inp14B');
      NodeCount := NumNodes;
      for NodeIndex := 0 to NumNodes -1 do
      begin
        I := NodeIndex+1+LineIndex;
        Splitter.DelimitedText := SutraFile[I];
        Nodes[NodeIndex].X := FortranStrToFloat(Splitter[2]);
        Nodes[NodeIndex].Y := FortranStrToFloat(Splitter[3]);
        if ThreeD then
        begin
          Nodes[NodeIndex].Z := FortranStrToFloat(Splitter[4]);
        end
        else
        begin
          Nodes[NodeIndex].Z := 0;
      ...
      LineIndex := SutraFile.IndexOf('#Start_inp22');
      ElementCount := NumElem;
      for ElementIndex := 0 to NumElem -1 do
      begin
        IElem := ElementIndex + LineIndex + 2;
        Splitter.DelimitedText := SutraFile[IElem];
        for NodeIndex := 0 to 7 do
        begin
          try
            NodeNum := StrToInt(Splitter[NodeIndex+1]);
            Elements[ElementIndex,NodeIndex] := NodeNum-1;
          except on E: Exception do
            begin
              ShowMessage(IntToStr(NodeIndex) + ' ' + IntToStr(ElementIndex));
              raise;
            end;
          end;
        end;
