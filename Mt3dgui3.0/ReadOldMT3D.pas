...
uses ReadOldUnit, Variables, ModflowUnit, MT3DFormUnit;
...
{        dgGeolSelectCell(nil, Ord(nuiSim), RowIndex, ABoolean);

        AComboBox := dgGeol.Objects[Ord(nuiTrans), RowIndex] as TComboBox;
...
//        AComboBox := dgGeol.Objects[Ord(nuiType), RowIndex] as TComboBox;
        if AnInteger = 0
        then
          begin
//           AComboBox.ItemIndex := AnInteger;
//           dtabGeol.CellSet.Item[RowIndex, Ord(uiType)].Value := 'Confined (0)';
           dgGeol.Cells[Ord(nuiType),RowIndex] := dgGeol.Columns[Ord(nuiType)].Picklist.Strings[0];
          end
        else if AnInteger = 1
        then
          begin
...
//           dtabGeol.CellSet.Item[RowIndex, Ord(uiType)].Value := 'Unconfined (1)';
           dgGeol.Cells[Ord(nuiType),RowIndex] := dgGeol.Columns[Ord(nuiType)].Picklist.Strings[1];
          end
        else
          begin
//           AComboBox.ItemIndex := 2;
//           dtabGeol.CellSet.Item[RowIndex, Ord(uiType)].Value := 'Convertible (3)';
           dgGeol.Cells[Ord(nuiType),RowIndex] := dgGeol.Columns[Ord(nuiType)].Picklist.Strings[3];
          end;
