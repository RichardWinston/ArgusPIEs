C     Last change:  RBW  21 Aug 2000    2:44 pm
...
C2------FLAG ISTRT (SHOULD STARTING HEADS BE KEPT FOR DRAWDOWN?).
      ...
      END IF
C RBW BEGIN CHANGE
C Always set ISTRT to 1 so data will be available for import
      ISTRT = 1
C RBW END CHANGE
...
C END ADDED CODE
C BEGIN MODIFIED CODE
C            WRITE(*,*) ' FIRST ENTRY IN NAME FILE MUST BE "LIST".'
C            STOP
C END MODIFIED CODE
         END IF
         IOUT=IU
...
C         WRITE(IOUT,*) ' BAS PACKAGE FILE HAS NOT BEEN OPENED.'
C         STOP
C END MODIFIED CODE
      END IF
      CLOSE(UNIT=INUNIT)
