...
{  TintegerArray = Array[0..32760] of LongInt;
  ...
  TfrmGrid = class(TArgusForm)
    ...
    btnPaste: TRbw95Button;
    WrapBtn1: TRbw95Button;
...
uses OptionsUnit, UtilityFunctions, LayerNamePrompt, ANECB, FixNameUnit;
...
begin
  ...
  begin
    result := FixArgusName(frmGrid.edDataLayerName.Text);
  end;
...
var
  Directory : string;
begin
  if GetDllDirectory(GetDLLName, Directory) then
  begin
    Application.HelpFile := Directory + '\GriddedImport.hlp';
  end;
  frmGrid := TfrmGrid.Create(Application);
  try
    frmGrid.HelpFile := Application.HelpFile;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ADataGrid.Cells[ColIndex, Index + 1] := ASubstring;
        Inc(ColIndex);
      end;
    end;
  end;
...
procedure TfrmGrid.WrapBtn1Click(Sender: TObject);
var
...
begin
  inherited;
  edDataLayerName.Enabled := not cbSeparateLayers.Checked;
  if cbSeparateLayers.Checked then
  begin
