...
// may actually be used.
  private
    Status : TStatus;
    FSetExpressionNow: boolean; // see TStatus.
...
//    function GetParentLayerHandle: ANE_PTR;
    ...
    function RenameIndexedParameter(const CurrentModelHandle : ANE_PTR;
      const LayerHandle : ANE_PTR) : ANE_BOOL;
    ...
    procedure SetExpression(const CurrentModelHandle : ANE_PTR;
      const LayerHandle : ANE_PTR);
    // SetExpression sets the expression of a parameter if SetValue is True.
  ...
  public
    ...
    // the parameter is created and should not be changed.
    SetValue : boolean;
    ...
    function WritePar(const aneHandle : ANE_PTR) : string;
    ...
    property SetExpressionNow : boolean read FSetExpressionNow write SetSetExpressionNow;
  end;
...
//    procedure DeleteParametersByName(AName : string); virtual;
    ...
    function RenameIndexedLayer(const CurrentModelHandle : ANE_PTR) : ANE_BOOL;
  ...
  public
    ...
    // LayerList the the layer list the owns the layer.
    Lock : TLayerLock;
    ...
    function WriteLayer(const CurrentModelHandle : ANE_PTR) : string;
    ...
    function GetPreviousLayer : T_ANE_MapsLayer;
    // GetPreviousLayer returns the layer after which it is appropriate to
    // add this layer.
    function GetLayerHandle(const CurrentModelHandle : ANE_PTR): ANE_PTR;
...
//    procedure DeleteParametersByName(AName : string); override;
    ...
    // layer in which SetValue is true.
...
T_ANE_DensityLayer = class(T_ANE_InfoLayer)
  ...
  public
//    constructor Create(Index: Integer; ALayerList : T_ANE_LayerList);
//      override;
    class Function ANE_LayerName : string ; override;
    // see T_ANE_MapsLayer
    function Units : string; override;
...
//    procedure DeleteByName(AName : string);
    ...
    function GetPreviousParameter(AParam : T_ANE_Param): T_ANE_Param;
    ...
    // in the T_ANE_ParameterList.
    function GetItem(Index : Integer) : T_ANE_Param;
...
//    function Add(Item: T_ANE_Param): Integer;
    // see TList
    function First: T_ANE_Param;
    // see TList
    function IndexOf(Item: T_ANE_Param): Integer;
    // see TList
//    procedure Insert(Index: Integer; Item: T_ANE_Param);
    // see TList
    function Last: T_ANE_Param;
    // see TList
//    function Remove(Item: T_ANE_Param): Integer;
    // see TList
    function GetParameterByName(AName : string) : T_ANE_Param;
    ...
    procedure AddOrRemoveParameter(AParameterType : T_ANE_ParamClass;
          ParameterName : string; ParameterShouldBePresent : boolean);
    ...
    procedure SetExpressionNow(AParameterType : T_ANE_ParamClass;
          ParameterName : string; ShouldSetExpressionNow : boolean);
    Property Items[Index : integer]: T_ANE_Param  read GetItem {write SetItem };
    // See TList
...
//    function Add(Item: T_ANE_IndexedParameterList): Integer;
    // see TList
    function First: T_ANE_IndexedParameterList;
    // see TList
    function IndexOf(Item: T_ANE_IndexedParameterList): Integer;
    // see TList
//    procedure Insert(Index: Integer; Item: T_ANE_IndexedParameterList);
    // see TList
    function Last: T_ANE_IndexedParameterList;
    // see TList
...
//    function Add(Item: T_ANE_MapsLayer): Integer;
    // see TList
    function First: T_ANE_MapsLayer;
    // see TList
    function IndexOf(Item: T_ANE_MapsLayer): Integer;
    // see TList
//    procedure Insert(Index: Integer; Item: T_ANE_MapsLayer);
    // see TList
    function Last: T_ANE_MapsLayer;
    // see TList
//    function Remove(Item: T_ANE_MapsLayer): Integer;
    // see TList
    function GetLayerByName(AName : string) : T_ANE_MapsLayer;
...
T_ANE_IndexedLayerList = class(T_ANE_LayerList)
  private
    ...
    // will call the inherited FreeByStatus.
    procedure RenameIndexedLayers(const CurrentModelHandle :ANE_PTR);
...
//    function Add(Item: T_ANE_IndexedLayerList): Integer;
    // see TList
    function First: T_ANE_IndexedLayerList;
    // see TList
    function IndexOf(Item: T_ANE_IndexedLayerList): Integer;
    // see TList
//    procedure Insert(Index: Integer; Item: T_ANE_IndexedLayerList);
    // see TList
    function Last: T_ANE_IndexedLayerList;
    // see TList
//    function Remove(Item: T_ANE_IndexedLayerList): Integer;
    // see TList
    function GetIndexedLayerListByName(AName : string) : T_ANE_IndexedLayerList;
...
//    procedure UpdateOldParameterIndicies;
  ...
  public
    ...
    // UnIndexedLayers, ListsOfIndexedLayers, and PostProcessingLayers.
    function MakeParameter1Lists(ParameterCount : integer) : TList ;
    ...
    function MakeParameter2Lists(ParameterCount : integer) : TList ;
    // MakeParameter1Lists creates a TList and then
    // cyles from 0 to ParameterCount -1. On each cycle it creates another
    // TList which is Added to the result-TList.
...
{    procedure SetUnIndexedExpressionNow(ALayerType : T_ANE_LayerClass;
    ...
    procedure OK(const CurrentModelHandle : ANE_PTR);
...
uses UtilityFunctions, CheckVersionFunction;
...
//  Units := '';
  ...
  if (AParameterList.ParameterOrder.Count > 0) and
     (AParameterList.ParameterOrder.IndexOf(self.Ane_ParamName) = -1)
  then
    begin
...
{    begin
  ...
  if plName in Lock then
  begin
    result := result +  'Lock Name';
    First_Lock := False;
  end;
  if plUnits in Lock then
  begin
  ...
  if plDont_Override in Lock then
  begin
    if not First_Lock then
    begin
      result := result + ', ';
  ...
  if plInhibit_Delete in Lock then
  begin
    if not First_Lock then
    begin
      result := result + ', ';
  ...
  if (plKind in Lock) and CheckArgusVersion(CurrentModelHandle,4,2,0,'d') then
  begin
    if not First_Lock then
    begin
      result := result + ', ';
...
Chr(10) is the new-line character.
  ...
  result :=
    ...
    Chr(9) + '{' + Chr(10) +
    Chr(9) + Chr(9) + 'Name: "' + WriteSpecialBeginning + WriteName
    ...
    Chr(9) + Chr(9) + 'Value: "' + Value + '"' + Chr(10) +
...
//  LayerHandle := GetParentLayerHandle;
  ...
  then
    ...
    begin
      ...
      then
        begin
          ParameterName := WriteSpecialBeginning + WriteName
...
//      ANE_ParameterName := PChar(ParameterName);
...
// RenameIndexedParameter changes the name of a parameter owned by a
// T_ANE_IndexedParameterList
function T_ANE_Param.RenameIndexedParameter(const CurrentModelHandle : ANE_PTR;
  const LayerHandle : ANE_PTR) : ANE_BOOL;
var
//  LayerHandle : ANE_PTR;
  ParameterIndex : ANE_INT32;
...
//  ProcessEvents(CurrentModelHandle);
  if (WriteIndex = WriteOldIndex) and not (Status = sNormal)
  then
    begin
        result := False;
    end
      ...
      then
        ...
        begin
          ...
          then
            ...
            begin
              NewParameterName := ANE_ParamName + WriteIndex;
...
begin
  ParentLayer := GetParentLayer;
  result := ParentLayer.ANE_LayerName ;
end;
...
//  Units := '';
  ...
  if (Index < 0) or (Index > LayerList.Count-1)
  then
    begin
...
// to return other values as appropriate.
  result :=
    'Layer:' + Chr(10) +
    '{' + Chr(10) +
    Chr(9) + 'Name: "' + WriteSpecialBeginning + ANE_LayerName
      + WriteSpecialMiddle + WriteIndex + WriteSpecialEnd + '"' + Chr(10) +
...
begin
  ...
  begin
    LayerName := WriteSpecialBeginning + ANE_LayerName
        + WriteSpecialMiddle + WriteIndex + WriteSpecialEnd;
...
// RenameIndexedLayer changes the name of the layer in Argus ONE from
// Name + OldIndex to Name + NewIndex.
function T_ANE_MapsLayer.RenameIndexedLayer(const CurrentModelHandle : ANE_PTR)
  : ANE_BOOL;
var
  LayerHandle : ANE_PTR;
  NewLayerName : string;
  ...
  then
    ...
    begin
      ...
      then
        ...
        begin
          NewLayerName := WriteSpecialBeginning + ANE_LayerName
            + WriteSpecialMiddle + WriteIndex + WriteSpecialEnd;
...
begin
  ...
  IndexedParamList1.RenameIndexedParameters(CurrentModelHandle, LayerHandle);
  IndexedParamList2.RenameIndexedParameters(CurrentModelHandle, LayerHandle);
end;
...
begin
  ...
  begin
    AT_ANE_Param := FList.Items[ParamIndex];
    if (AT_ANE_Param.Status = sDeleted) then
    begin
...
// parameters in the T_ANE_IndexedParameterList from
// Name + OldIndex to Name + Index.
procedure T_ANE_IndexedParameterList.RenameIndexedParameters
  (const CurrentModelHandle :ANE_PTR; const LayerHandle : ANE_PTR);
var
  AT_ANE_Param : T_ANE_Param;
  ParamIndex : integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AT_ANE_Param.RenameIndexedParameter(CurrentModelHandle, LayerHandle );
        end;
      end
    end;
...
// the T_ANE_IndexedParameterList in the T_ANE_ListOfIndexedParameterLists.
procedure T_ANE_ListOfIndexedParameterLists.RenameIndexedParameters
  (const CurrentModelHandle : ANE_PTR; const LayerHandle : ANE_PTR);
var
  AParamList : T_ANE_IndexedParameterList;
  ParamListIndex : integer;
begin
  ...
  begin
    ...
    begin
      AParamList.RenameIndexedParameters(CurrentModelHandle, LayerHandle );
    end;
  end;
  for ParamListIndex := 0 to Count -1 do
  begin
...
var
  ALayer : T_ANE_Layer;
begin
  ALayer := self.GetLayerByName(ALayerType.ANE_LayerName) as ALayerType;
...
procedure T_ANE_IndexedLayerList.RenameIndexedLayers
  (const CurrentModelHandle :ANE_PTR);
var
  ALayer : T_ANE_MapsLayer;
  LayerIndex : integer;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ALayer.RenameIndexedLayer(CurrentModelHandle);
      end
    end;
  end;
end;
...
// RenameIndexedLayers calls the RenameIndexedLayers procedure of all the
// T_ANE_IndexedLayerList's in the T_ANE_ListOfIndexedLayerLists.
procedure T_ANE_ListOfIndexedLayerLists.RenameIndexedLayers
  (const CurrentModelHandle : ANE_PTR);
var
  ALayerList : T_ANE_IndexedLayerList;
  LayerListIndex : integer;
begin
  ...
  begin
    ...
    begin
      ALayerList.RenameIndexedLayers(CurrentModelHandle);
    end;
  end;
  for LayerListIndex :=  0 to Count -1 do
  begin
...
// ListsOfIndexedLayers resulting in all indexed layers in the layer
// structure having their names updated following removal of deleted layers.
procedure TLayerStructure.RenameIndexedLayers
  (const CurrentModelHandle : ANE_PTR);
begin
  ListsOfIndexedLayers.RenameIndexedLayers(CurrentModelHandle);
end;
  ...
  try
    begin
      ...
      begin
        ...
        LayerNames.Add(AMapLayer.ANE_LayerName + AMapLayer.WriteIndex);
...
begin
  ...
  try
    ...
    FreeByStatus(sDeleted);
    UpdateIndicies;
    RenameIndexedLayers(CurrentModelHandle);
    ANE_ProcessEvents(CurrentModelHandle);
