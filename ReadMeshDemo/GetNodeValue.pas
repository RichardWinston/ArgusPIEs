...
procedure GExportNodesPIE(const refPtX : ANE_DOUBLE_PTR      ;
...
begin
  ...
  try // try 1
    ...
    begin
      ...
      ProjectOptions := TProjectOptions.Create(funHandle);
      try  // try 2
        LayerHandle := ProjectOptions.GetLayerByName(LayerName);
        if LayerHandle <> nil then
        begin
          LayerOptions := TLayerOptions.Create(funHandle, LayerHandle);
          try // try 3
            ParamIndex := LayerOptions.GetParameterIndex(ParamName);
            if ParamIndex > -1 then
            begin
              AStringList := TStringList.Create;
              try
                for Index := 0 to QuadMesh.NodeList.Count -1 do
                begin
                  ANode := QuadMesh.NodeList[Index];
                  AStringList.Add(ANode.Values[ParamIndex]);
                end;
