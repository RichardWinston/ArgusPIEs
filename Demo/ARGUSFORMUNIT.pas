...
var
  ...
  RowIndex, ColIndex, MemoIndex
{$IFDEF StringGrid3d}
  , GridIndex
{$ENDIF}
  : integer; // indicies for loop over rows
...
begin
  ...
  {$IFDEF StringGrid3d}
          else if Components[ComponentIndex] is TRBWCustom3DGrid then
          begin
            ComponentData.Add(AComponent.Name);
            ComponentData.Add
...
begin
  ...
  StringToForm(DataToRead, UnreadData,
       ...
       FirstList, nil, PIEDeveloper);

  // read all other data.
  StringToForm(DataToRead, UnreadData,
       VersionControl, VersionInString, True, nil, FirstList,
       PIEDeveloper);
