...
C-------CONVERT CELL TO NO FLOW IF CELL THICKNESS IS 0.
      ...
      IF(IBOUND(J,I,K).GT.0) THEN
         ...
         IF(THICK.LE.ZERO) THEN
            ...
            HNEW(J,I,K)=HCNV
c rbw begin change
c            IF(LAYWT(K).NE.0) WETDRY(J,I,LAYWT(K))=ZERO
c            WRITE(IOUT,25) K,I,J
c   25       FORMAT(1X,
c     &  'Converting cell to no flow due to 0 thickness (Layer,row,col)',
c     &     I4,',',I4,',',I4)
c rbw end change
         END IF
      END IF
...
c     & ' ELIMINATED BECAUSE ALL HYDRAULIC CONDUCTIVITIES TO NODE ARE 0')
c   50    CONTINUE
c      END IF
c   60 CONTINUE
c rbw end change
