...
c rbw begin change
c bug fix
c             DO 19 ICLUSTER=IPLOC(1,NP),IPLOC(2,NP)
...
c rbw begin change
c      DO 60 K=1,NLAY
c      IF(LAYWET(K).NE.0) THEN
C
C2------WETTING IS ACTIVE.
c         DO 40 I=1,NROW
c         DO 40 J=1,NCOL
c         IF(IBOUND(J,I,K).EQ.0 .AND. WETDRY(J,I,LAYWET(K)).EQ.ZERO)
c     1                GO TO 40
C
...
c         IF(NLAY.GT.1) THEN
c            IF(VKA(J,I,K).NE.ZERO) THEN
c               IF(K.NE.NLAY .AND. LAYCBD(K).NE.0) THEN
c                  IF(VKCB(J,I,LAYCBD(K)).NE.ZERO) GO TO 40
c               END IF
c               IF(K.NE.1 .AND. LAYCBD(K-1).NE.0) THEN
c                  IF(VKCB(J,I,LAYCBD(K-1)).NE.ZERO) GO TO 40
c               END IF
c            END IF
c         END IF
C
...
c            IF(VKA(J,I,K).NE.ZERO) THEN
c               IF(K.NE.NLAY .AND. LAYCBD(K).NE.0) THEN
c                  IF(VKCB(J,I,LAYCBD(K)).NE.ZERO) GO TO 50
c               END IF
c               IF(K.NE.1 .AND. LAYCBD(K-1).NE.0) THEN
c                  IF(VKCB(J,I,LAYCBD(K-1)).NE.ZERO) GO TO 50
c               END IF
c            END IF
c         END IF
C
...
c         HNEW(J,I,K)=HCNV
c         WRITE(IOUT,43) K,I,J
c   43    FORMAT(1X,'NODE (LAYER,ROW,COL) ',I3,2(1X,I5),
c     1 ' ELIMINATED BECAUSE ALL HYDRAULIC',/,
c     2 ' CONDUCTIVITIES TO NODE ARE 0')
c   50    CONTINUE
c      END IF
c   60 CONTINUE
c rbw end change
