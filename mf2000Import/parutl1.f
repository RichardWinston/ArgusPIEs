...
C     type, and value
      ...
      CTMP1=PN
c rbw begin change
      CurrentNameCount = CurrentNameCount + 1
      if (CurrentNameCount.gt.MyMaxPar) then
        WRITE(IOUT,*) ' Too many parameter names in use.'
        CALL USTOP(' ')
      END IF
      CurrentNames(CurrentNameCount) = CTMP1
      CurrentInstance(CurrentNameCount) = ' '
c rbw end change
      CALL UPCASE(CTMP1)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
...
c RBW begin change
      Module StoredName
        character*11 ParamName
      end Module StoredName

      subroutine GetHFB_ParamName(AName)
      use StoredName
      dll_export GetHFB_ParamName
      character( len=*) :: AName
        ANAME = ParamName
      end
c RBW end change
...
c RBW begin change
      use StoredName
      dll_import CreateNewHfbParameter
c RBW end change
      INCLUDE 'param.inc'
      CHARACTER*(*) PACK,PTYPX
      CHARACTER*4 PTYP, HFB_PTYPE
      CHARACTER*10 PN,CTMP1,CTMP2
      character PN2
      CHARACTER*200 LINE
C     ------------------------------------------------------------------
...
C         Parameter was not found in the list, so it is a new definition.
          ...
          IPSUM=IPSUM+1
c RBW begin change
          if (PTYP.eq.HFB_PTYPE) then
            ParamName = Char(10) // PN
            CALL CreateNewHfbParameter(PV)
          endif
c RBW end change
          GO TO 100
        ENDIF
...
c rbw begin change
      use CurrentParameters
      use StoredName
      dll_import ReadCurrentHfbParameter
c rbw end change
      INCLUDE 'param.inc'
      CHARACTER*(*) PACK,PTYP
...
c RBW end change
    ...
    1 FORMAT(/,' Parameter:  ',A)
      ParamName = Char(10) // LINE(ISTART:ISTOP)
      IF(LINE(ISTART:ISTOP).EQ.' ') THEN
        WRITE(IOUT,*) ' Blank parameter name in the ',PACK,' file.'
        CALL USTOP(' ')
      END IF
...
c rbw end change
   ...
   81       FORMAT(1X,'Parameter type conflict:',/
     ...
     3          ' file, so it should be type:',A)
          ...
          END IF
c rbw begin change
          IF (PTYP.EQ.HFB_PTYPE) CALL ReadCurrentHfbParameter()
c rbw end change
C
C         DESIGNATE CELLS CORRESPONDING TO CORRECT PARAMETER INSTANCE
 ...
 1010       FORMAT(3X,'Instance:  ',A)
c rbw begin change
            CurrentInstance(CurrentNameCount) = CTMP3
c rbw end change
            CALL UPCASE(CTMP3)
            DO 50 KI=1,NUMINST
              CTMP4=INAME(ILOC+KI-1)
...
c rbw begin change
          CellLimits(1,CurrentNameCount) = IPLOC(1,IP)+(NI-1)*NLST
          CellLimits(2,CurrentNameCount) =
     *       NLST-1+IPLOC(1,IP)+(NI-1)*NLST
C rbw end change
C  Substitute values
          DO 90 I=1,NLST
            II=NTOT-NLST+I
   ...
   85       CONTINUE
c rbw begin change
c don't use parameter value to modify value for cell
c            DO 86 IPVL=IPVL1,IPVL2
c              RLIST(IPVL,II)=RLIST(IPVL,II)*B(IP)
c   86       CONTINUE
c rbw end change
            IL=RLIST(1,II)
            IR=RLIST(2,II)
