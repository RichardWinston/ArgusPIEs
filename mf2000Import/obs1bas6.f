...
C     ------------------------------------------------------------------
     ...
     &          ISSA(NPER)
      ...
      REAL OTIME(ND)
c rbw begin change
      dll_import CreateNewHeadObs, ReadLayerProportion, ReadITT,
     &  ReadHeadObsTime
c rbw end change
...
c rbw begin change
              call ReadHeadObsTime(OBSNAM(N1), IREFSP, TOFFSET,
     &                       HOBS(N1), WT(N1), DUM, IWT, IPLOT(N1))
c rbw end change
              NAMES(N1) = OBSNAM(N1)
...
C----------READ INFORMATION UNIQUE TO THIS OBSERVATION (ITEM 6)
     ...
     &                     WT2, IWT, IPLOT(N)
c rbw begin change
              call ReadHeadObsTime(OBSNAM(N), IREFSP, TOFFSET,
     &                       HOBS(N), WT1, WT2, IWT, IPLOT(N))
c rbw end change
            NAMES(N) = OBSNAM(N)
...
c rbw begin change
          call CreateNewHeadObs(OBSNAM(N), NDER(1,N), NDER(2,N),
     &      NDER(3,N), IREFSP, TOFFSET,
     &      ROFF(N), COFF(N), HOBS(N), WT(N), IWT, IPLOT(N))
c rbw end change
        NAMES(N) = OBSNAM(N)
        WRITE (IOUT,535) N, OBSNAM(N), IREFSP, TOFFSET, HOBS(N),
...
C-------READ INFORMATION FOR MULTILAYER OBSERVATIONS (ITEM 4)
        IF (K.LT.0) THEN
          ...
          do M=1, -K
            call ReadLayerProportion(MLAY(M,ML),PR(M,ML))
          enddo
          MM = -K
          IF (MM.GT.KL) KL = MM
          TPR = 0.0
