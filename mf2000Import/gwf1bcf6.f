...
C1------MULTIPLY VERTICAL LEAKANCE BY AREA TO MAKE CONDUCTANCE.
      ...
      DO 10 I=1,NROW
      DO 10 J=1,NCOL
c      CV(J,I,K)=CV(J,I,K)*DELR(J)*DELC(I)
   10 CONTINUE
C
...
c rbw begin change
c      DO 60 K=1,NLAY
c      IF(LAYCON(K).EQ.1 .OR. LAYCON(K).EQ.3) GO TO 50
C
C4A-----WHEN LAYER TYPE IS 0 OR 2, TRANSMISSIVITY OR CV MUST BE NONZERO.
c      DO 45 I=1,NROW
c      DO 45 J=1,NCOL
c      IF(IBOUND(J,I,K).EQ.0) GO TO 45
c      IF(CC(J,I,K).NE.ZERO) GO TO 45
c      IF(K.EQ.NLAY) GO TO 41
c      IF(CV(J,I,K).NE.ZERO) GO TO 45
c   41 IF(K.EQ.1) GO TO 42
c      IF(CV(J,I,K-1).NE.ZERO) GO TO 45
c   42 IBOUND(J,I,K)=0
c      HNEW(J,I,K)=HCNV
...
c      IF(IWDFLG.EQ.0) GO TO 55
c      IF(WETDRY(J,I,KB).EQ.ZERO) GO TO 55
c      IF(K.EQ.NLAY) GO TO 51
c      IF(CVWD(J,I,K).NE.ZERO) GO TO 59
c   51 IF(K.EQ.1) GO TO 57
c      IF(CVWD(J,I,K-1).NE.ZERO) GO TO 59
c      GO TO 57
C
C4B2----WETTING CAPABILITY IS INACTIVE, SO CHECK CV AT ACTIVE CELLS.
c   55 IF(IBOUND(J,I,K).EQ.0) GO TO 59
c      IF(K.EQ.NLAY) GO TO 56
c      IF(CV(J,I,K).NE.ZERO) GO TO 59
c   56 IF(K.EQ.1) GO TO 57
c      IF(CV(J,I,K-1).NE.ZERO) GO TO 59
C
C4B3----CHECK HYDRAULIC CONDUCTIVITY.
c   57 IF(HY(J,I,KB).NE.ZERO) GO TO 59
C
C4B4----HY AND CV ARE ALL 0, SO CONVERT CELL TO NO FLOW.
c      IBOUND(J,I,K)=0
c      HNEW(J,I,K)=HCNV
c      IF(IWDFLG.NE.0) WETDRY(J,I,KB)=ZERO
c      WRITE(IOUT,43) K,I,J
c   59 CONTINUE
c   60 CONTINUE
C
