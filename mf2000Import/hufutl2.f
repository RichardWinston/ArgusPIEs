...
c rbw begin change
      integer ICluster, ZoneCount
      integer Zones(10)
      dll_import ReadNewHufParameter, ReadNewHufCluster
c rbw end change
C     ------------------------------------------------------------------
C
      ILFLG=1
...
c rbw begin change
       APARAMNAME = CTMP1
       APARAMTYP = PTYP
      call ReadNewHufParameter(PV, NDHUF)
c rbw end change
      IACTIVE(NP)=0
C
...
c rbw begin change
      AUnitName = CTMP1
      AMultName = LINE(IM1:IM2)
      AZoneName = LINE(IZ1:IZ2)
      ZoneCount = 0
c rbw end change
      DO 30 J=5,14
...
c rbw end change
   ...
   37    FORMAT(1X,'               MULTIPLIER:',A,'   ZONE:',A)
      END IF
c rbw begin change
      call ReadNewHufCluster(ICluster, Zones)
c rbw end change
C
