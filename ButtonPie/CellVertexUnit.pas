...
//    Angle : double;
    ...
    Segment : double;  // Segment is a number indicating the line segment on
    ...
    LastVertex: Boolean; // set to true if this is the last vertex in a line
    Constructor Create;
...
var
  RowCenter : TReal;
begin
  RowCenter := RowMiddleList.Items[Row - 1];
  Result := RowCenter.Value ;
end;
...
var
  ColumnCenter : TReal;
begin
  ColumnCenter := ColumnMiddleList.Items[Column - 1];
  Result := ColumnCenter.Value ;
end;
...
begin
      result := SegmentSecondVertexXPos(SegmentIndex) - SegmentFirstVertexXPos(SegmentIndex);
end;
...
begin
      result := SegmentSecondVertexYPos(SegmentIndex) - SegmentFirstVertexYPos(SegmentIndex);
end;
...
begin
      result := Sqrt(Sqr(SegmentXLength(SegmentIndex)) + Sqr(SegmentYLength(SegmentIndex)));
end;
...
begin
  ...
  if ARow.Value = RowCenter
  then
    begin
      result := (not (UpperY < RowCenter) and not (LowerY > RowCenter) and not (UpperY = LowerY) );
    end
  else
    begin
...
begin
  ...
  if AColumn.Value = ColumnCenter
  then
    begin
      result := (not (UpperX < ColumnCenter) and not (LowerX > ColumnCenter) and not (UpperX = LowerX));
    end
  else
    begin
...
begin
  if SegmentSecondVertexYPos(SegmentList.Count -1) > RowCenter
  then
    begin
...
begin
  if SegmentSecondVertexXPos(SegmentList.Count -1) > ColumnCenter
  then
    begin
