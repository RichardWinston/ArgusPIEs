...
type
  ...
  TContourType = (ctPoint, ctOpen, ctClosed);
  ...
  private
    ...
    function GetCopyDelimiter(ModelHandle : ANE_PTR): String;
    ...
    procedure SetCopyDelimiter(ModelHandle : ANE_PTR; const Value: String);
...
//  published
    ...
    property CopyDelimiter[ModelHandle : ANE_PTR]	       	: String read GetCopyDelimiter write SetCopyDelimiter;		// the document's preference	for the delimiter at contour copy
...
{    Constructor Create(ModelHandle : ANE_PTR; const layerHandle : ANE_PTR;
  ...
  public
    ...
    function ContourType(ModelHandle : ANE_PTR) : TContourType;
...
function TProjectOptions.GetCopyDelimiter(ModelHandle : ANE_PTR): string;
Var
  AneResult : ANE_STR;
begin
...
procedure TProjectOptions.SetCopyDelimiter(ModelHandle : ANE_PTR; const Value: string);
Var
  ANEValue : ANE_STR;
begin
...
begin
  result := Rename(ModelHandle, PChar(NewName));
end;
...
var
  Expression: String;
begin
  Expression := Format('NthParamName(%d)', [ParameterIndex]);
  result := StringValueAtXY(ModelHandle, 0, 0, Expression);
end;
...
{function TLayerOptions.IntValueAtXY(ModelHandle : ANE_PTR; X, Y: ANE_DOUBLE;
  Expression: String): ANE_INT32;
var
  LocalX, LocalY : ANE_DOUBLE;
  LocalResult : ANE_INT32;
begin
  if Expression <> '' then
  begin
//    FreeParsedExpression;
...
function TContourObjectOptions.ContourType(ModelHandle : ANE_PTR): TContourType;
var
  NodeCount : integer;
